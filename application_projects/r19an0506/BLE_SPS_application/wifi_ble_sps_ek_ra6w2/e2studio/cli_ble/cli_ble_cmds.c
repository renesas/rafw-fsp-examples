...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_SVC_START};

    RM_BLE_MSG_QUEUE_Enqueue(&msg);
...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_SVC_STOP};
...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_ADV_START};
...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_ADV_STOP};
...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_DISCONNECT};
...
{
    rm_ble_msg_t msg = {.type = RM_BLE_MSG_TYPE_BD_ADDR_GET};
...
#else
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_BLECFG, BLE_CFG_BD_ADDRESS, &bd_addr);
#endif
    if (NULL != bd_addr)
    {
        AUTOTEST_PRINT("BD address from NVRAM: %s\n", bd_addr);
    }
}
...
bool cmd_ble_write_public_bd_addr (int argc, char * argv[])
{
    if ((argc < 2) || (argc > 3))
    {
        AUTOTEST_PRINT("usage: ble set_bd_addr <aa:bb:cc:dd:ee:ff>\n");
    ...
    {
        AUTOTEST_PRINT(
            "usage: ble set_bd_addr <aa:bb:cc:dd:ee:ff> - addr should be 6 hex represented bytes, each limited by ':'");

        return false;
    }

    bool         contain_non_hex = false;
    const char * in_bd_addr      = argv[1];
    ...
    for (uint32_t i = 0; i < strlen(argv[1]); i++)
    {
        ...
        if (((ch < '0') || (ch > '9')) &&
            ((ch < 'A') || (ch > 'F')))
        {
            AUTOTEST_PRINT("BD address is expected as a hex string, example: aa:bb:cc:dd:ee:ff\n");
    ...
    {
        AUTOTEST_PRINT("BD address is expected as a string with 5 limiters ':'\n");
