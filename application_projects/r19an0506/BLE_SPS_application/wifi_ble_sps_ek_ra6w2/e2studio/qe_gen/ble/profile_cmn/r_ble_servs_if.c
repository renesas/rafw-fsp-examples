            ...
            for (uint8_t d = 0; d < p_char->num_of_descs; d++)
            {
                    ...
                    (NULL != p_char->pp_descs[d]->decode))
                {
                    ...
                    else
                    {
                        ...
                        p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->pp_descs[d]->desc_idx,
                                                              p_char->pp_descs[d]->inst_idx, BLE_SERVS_WRITE_REQ),
                                   BLE_SUCCESS,
                                   &evt_data);
                    }
        ...
        if (NULL != p_char->pp_descs)
        {
            for (uint8_t d = 0; d < p_char->num_of_descs; d++)
            {
                    ...
                    (NULL != p_char->pp_descs[d]->decode))
                {
                    ...
                    gatt_value.value_len = p_char->pp_descs[d]->db_size;
                    gatt_value.p_value   = qe_ble_malloc(p_char->pp_descs[d]->db_size);
                    ...
                    else
                    {
                        ...
                        p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->pp_descs[d]->desc_idx,
                                                              p_char->pp_descs[d]->inst_idx, BLE_SERVS_READ_REQ),
                                   BLE_SUCCESS,
                                   &evt_data);
                    }

                    qe_ble_free(gatt_value.p_value);
                    qe_ble_free(p_app_value);
                    break;
                }
            }
    ...
    switch (db_op)
    {
        case BLE_GATTS_OP_CHAR_PEER_READ_REQ:
        {
            ...
            else
            {
                ...
                p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx, p_char->inst_idx, BLE_SERVS_READ_REQ),
                           BLE_SUCCESS,
                           &evt_data);
            }
        ...
        case BLE_GATTS_OP_CHAR_PEER_WRITE_REQ:
        {
            if (NULL != p_char->decode)
            {
                ...
                p_app_value = qe_ble_malloc(p_char->app_size);
                ...
                else
                {
                    ...
                    p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx, p_char->inst_idx, BLE_SERVS_WRITE_REQ),
                               ret,
                               &evt_data);
                }
        ...
        case BLE_GATTS_OP_CHAR_PEER_WRITE_CMD:
        {
            if (NULL != p_char->decode)
            {
                ...
                else
                {
                    ...
                    p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx, p_char->inst_idx, BLE_SERVS_WRITE_CMD),
                               ret,
                               &evt_data);
                }
            ...
            if ((NULL != p_serv) && (NULL != p_char))
            {
                if ((p_char->start_hdl + 1) == p_cfm_evt_param->attr_hdl)
                {
                    ...
                    else
                    {
                        ...
                        p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx, p_char->inst_idx,
            ...
            if ((NULL != p_serv) && (NULL != p_char))
            {
                ...
                if (((p_char->start_hdl + 1) == p_write_rsp_evt_param->attr_hdl) && (NULL != p_char->decode))
                {
...
#if QE_BLE_READ_DATA_WHEN_WRITE_RSP_COMP == 1
                    p_app_value = qe_ble_malloc(p_char->app_size);
                    ...
                    else
                    {
                        ...
                        p_serv->cb(BLE_SERVS_MULTI_ATTR_EVENT(p_char->char_idx, p_char->inst_idx, BLE_SERVS_WRITE_COMP),
                                   ret,
                                   &evt_data);
                    }
                    ...
                    if (NULL != p_char->pp_descs)
                    {
                        for (uint8_t d = 0; d < p_char->num_of_descs; d++)
                        {
                                ...
                                (NULL != p_char->pp_descs[d]->decode))
                            {
                                ...
                                {
                                    p_char->pp_descs[d]->write_comp_cb(&p_char->pp_descs[d],
                                                                       p_data->conn_hdl,
                                                                       ret,
                                                                       p_app_value);
                                }
...
#if (BSP_CFG_RTOS == 1)                /* ThreadX */
    UINT status = tx_byte_pool_create(&g_qe_byte_pool, "QE_SERVS_HEAP", &g_qe_byte_pool_buff[0], QE_BLE_BYTE_POOL_LEN);
                                     ...
                                     bool                             is_notify)
{
    ...
    void * p_gatt_value = qe_ble_malloc(p_attr->db_size);
...
ble_status_t R_BLE_SERVS_GetChar (const st_ble_servs_char_info_t * p_attr, uint16_t conn_hdl, void * p_app_value)
{
    ...
    gatt_value.p_value   = qe_ble_malloc(p_attr->db_size);
