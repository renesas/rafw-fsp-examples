...
5. Verification:
 ...
 12. After reboot RA6W2 will connect to the configured AP.
 13. After connection to AP is successfull, RA6W2 will check for the AWS IoT provisioned credentials. If it is available,
     then RA6W2 will directly go for AWS IoT Core connection using the provisioned credentials. If not, then RA6W2 will
     initiate the Fleet Provisioning by Claim process. Since this is inital bootup, RA6W2 will go for Fleet Provisioning.
 14. After Fleet provisioing is completed, the unique certificate got from AWS Core and unique key generated locally will be 
     saved in the nvram and RA6W2 will disconnect itself from AWS IoT Core and reconnect using the provisioned 
     credentials. 
 ...
 16. Now since the fleet provisioning is now completed, if RA6W2 is rebooted, it will directly connects to AWS IoT Core using
