...
void provisioning_gpio_fr_handler(void *param);
...
void provision_app_gpio_handle_task_start(void)
{
    ...
    status = xTaskCreate(provision_app_gpio_event_task,
    ...
    if (status!=pdPASS)
    {
        APP_PRINT_INFO("Provisioning task creation failed ....\n");
    }
}
