...
#include "inc/provisioning_tls_key.h"
...
int provisioning_tls_svr_rsa_decrypt_func(void  *ctx,
                                          ...
                                          size_t output_max_len)
{
    return mbedtls_rsa_pkcs1_decrypt((mbedtls_rsa_context *) ctx, NULL,
                                         NULL, olen, input, output, output_max_len);
}
...
int provisioning_tls_svr_init_socket(provisioning_tls_server_cfg_t *config)
{
    ...
    snprintf(str_port, sizeof(str_port), "%u", config->local_port);
...
error: provisioning_tls_svr_deinit_ssl(config);
