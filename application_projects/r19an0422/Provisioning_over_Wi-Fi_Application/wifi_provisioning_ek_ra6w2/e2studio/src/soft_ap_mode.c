...
static void usr_api_load_params(struct setup_params *params)
{
    ...
    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_SSID_1, &result_ptr);
    if (result_ptr)
    ...
    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_ENCKEY_1, &result_ptr);
    if (result_ptr)
        strcpy(params->password[1], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_SAE_GROUPS_0, &result_ptr);
    if (result_ptr)
        strcpy(params->sae_groups[0], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_SAE_GROUPS_1, &result_ptr);
    if (result_ptr)
        strcpy(params->sae_groups[1], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_WEPKEY0_0, &result_ptr);
    if (result_ptr)
        strcpy(params->wep_key, result_ptr);

    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_WEPINDEX_0, (int *)&params->wep_key_idx);
    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_WEPTYPE_0, (int *)&params->wep_key_type);

    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_EAP_AUTH_MODE, (int *)&params->eap_auth_mode);
    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_EAP_PHASE2, (int *)&params->eap_phase2);
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_EAP_ID, &result_ptr);
    if (result_ptr && strlen(result_ptr) > 0)
            strncpy(params->eap_id, result_ptr, strlen(result_ptr));

    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_EAP_PW, &result_ptr);
    if (result_ptr && strlen(result_ptr) > 0)
        	strncpy(params->eap_pw, result_ptr, strlen(result_ptr));

    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_P2P_LISTEN_CH, (int *)&params->p2p_listen_chan);
    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_P2P_GO_INTENT, (int *)&params->p2p_go_intent);

    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_NETMODE_0, (int *)&params->netmode[0]);
    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                 WIFI_PROFILE_NETMODE_1, (int *)&params->netmode[1]);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_IPADDR_0, &result_ptr);
    if (result_ptr)
        	strcpy(params->ipaddress[0], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_IPADDR_1, &result_ptr);
    if (result_ptr)
        	strcpy(params->ipaddress[1], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_NETMASK_0, &result_ptr);
    if (result_ptr)
        	strcpy(params->subnetmask[0], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_NETMASK_1, &result_ptr);
    if (result_ptr)
        	strcpy(params->subnetmask[1], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_GATEWAY_0, &result_ptr);
    if (result_ptr)
        	strcpy(params->gateway[0], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_GATEWAY_1, &result_ptr);
    if (result_ptr)
        	strcpy(params->gateway[1], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
    		                        WIFI_PROFILE_DNSSVR_0, &result_ptr);
    if (result_ptr)
        	strcpy(params->dns[0], result_ptr);

    result_ptr = NULL;
    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_DNSSVR_1, &result_ptr);
    if (result_ptr)
...
void execute_AP_profile(void)
{
        ...
        !(params->security[WLAN1_IFACE] == eWiFiSecurityWPA3_192B_ent_ext))
    {
        ...
        memcpy(net_params->xNetworkParams.xPassword.xWPA.cPassphrase, params->password[WLAN1_IFACE],
    ...
    if (wifi_err)
        APP_PRINT_INFO("%s: WIFI_ConfigureAPExt failed with wifi_err=%d\n", __func__, wifi_err);

    wifi_err = WIFI_StartAP();
    if (wifi_err)
        APP_PRINT_INFO("%s: WIFI_StartAP failed with wifi_err=%d\n", __func__, wifi_err);

    APP_PRINT_INFO("Start AP...\n");
