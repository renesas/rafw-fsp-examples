...
#define AWSIOT_PROVISIONING_MODE_GENERAL            10
#define AWSIOT_PROVISIONING_MODE_AT                 11

#define AWSIOT_THINGNAME_JSON_LEN                   192
#define AWSIOT_PROVISIONING_MODE_JSON_LEN           21
...
static int32_t jsoneq(const char *json, jsmntok_t *tok, const char *s)
{
    ...
    if (tok->type == JSMN_STRING && (uint32_t)strlen(s) == (size_t)tok_len &&
        strncmp(json + tok->start, s, (size_t)tok_len) == 0)
    {
      return 0;
    }
...
{
    WIFI_STATUS_t notify_status = WIFI_CMD_AWS;
    st_ble_gatt_value_t gatt_value;
    char json_value[AWSIOT_PROVISIONING_MODE_JSON_LEN];
    ble_status_t ble_status = (ble_status_t)notify_status;

    UNUSED(params);
    LOG_INFO("prov_cmd_get_mode_t: {}\n");
    snprintf(json_value, sizeof(json_value), "{\"mode\":%d}", AWSIOT_PROVISIONING_MODE_GENERAL);
    ...
    R_BLE_GATTS_SetAttr(BLE_GAP_INVALID_CONN_HDL,
                        QE_ATTRIBUTE_HANDLE_CHARACTERISTIC_VALUE_WIFIPROVISIONING_AWS_IOT,
                        &gatt_value);
...
static thing_name_info_t get_thing_name(void)
{
   ...
   {
       info.thingName   = getMemoryString((const char*)AWS_IOT_THING_NAME);
...
static void params_cmd_get_name_print(const prov_cmd_get_name_t *params)
{
   WIFI_STATUS_t notify_status = WIFI_CMD_AWS;
   st_ble_gatt_value_t gatt_value;
   char json_value[AWSIOT_THINGNAME_JSON_LEN];
   ble_status_t ble_status = (ble_status_t)notify_status;
...
static void network_info(prov_cmd_network_info_t *params)
{
    ...
    ble_status_t ble_status = (ble_status_t)status;
    st_ble_wifiprovisionings_wifiprovisoning_t prov_value;

    params_cmd_network_info_print(params);
    memcpy(&provisioninig_network_info, params, sizeof(provisioninig_network_info));

    /* ----- Prepare JSON response ----- */
    strncpy(prov_value.value,
            json_generic_response(WIFI_PROV_NETWORK_INFO),
            sizeof(prov_value.value));

    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(&prov_value);
    /* ----- WiFi status notify ----- */
    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &ble_status);
}
...
static void select_ap(prov_cmd_select_ap_t *params)
{
    ...
    st_ble_wifiprovisionings_wifiprovisoning_t prov_msg;
    ble_status_t notify_value = (ble_status_t)notify_status;

    params_cmd_select_ap_print(params);
    provisioning_cb.wifi_connect(params);
    memset(&prov_msg, 0, sizeof(prov_msg));
    strncpy(prov_msg.value, json_generic_response(status),
            sizeof(prov_msg.value) - 1);
    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(&prov_msg);
    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &notify_value);
...
static void scan()
{
    ...
    if (rc == 0)
    {
        ...
        for (uint32_t i = 0; i < networks->num_of_networks; i++)
        {
            ...
            LOG_INFO("SSID: %25s, RSSI: %" PRId32 ", Channel: %" PRIu32 ", Security: %d\n",
    ...
    ble_status = (ble_status_t) status;
    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &ble_status);
}
...
static void reboot()
{
    ...
    ble_status_t ble_status = (ble_status_t) status;
    const char *json;
    st_ble_wifiprovisionings_wifiprovisoning_t value = {0};

    json = json_generic_response(WIFI_PROV_REBOOT_ACK);
    strncpy(value.value, json, sizeof(value.value) - 1);
    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(&value);
    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &ble_status);
...
static void check_network()
{
    ...
    st_ble_wifiprovisionings_wifiprovisoning_t value = {0};
    ble_status_t ble_notify = (ble_status_t) notify_status;
    ...
    json = json_check_network_response(status,
                                       provisioning_data.ssid,
                                       provisioning_data.password,
                                       provisioning_data.security_type);
    strncpy(value.value, json, sizeof(value.value) - 1);
    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(&value);
    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &ble_notify);
}

static void network_info_event_handler(cmd_t param,
                                       const char *cmd, jsmntok_t *token,
                                       uint32_t num_of_elements)
{
    ...
    for (uint32_t i = 0; i < num_of_elements; i++)
    {
        ...
        {
            /* String */
            cmd_str_start = cmd + token[i+1].start;
            len = (uint32_t)(token[i+1].end - token[i+1].start);
        ...
        {
            cmd_str_start = cmd + token[i+1].start;
            value = strtol(cmd_str_start, &endptr, 10);
...
static void generic_event_handler(cmd_t param, const char *cmd,
                                  jsmntok_t *token, uint32_t num_of_elements)
{
    UNUSED(cmd);
    UNUSED(token);
...
void provisioning_cmd_dispatch(char *cmd)
{
    ...
    num_of_elements = jsmn_parse(&parser, cmd, strlen(cmd),
