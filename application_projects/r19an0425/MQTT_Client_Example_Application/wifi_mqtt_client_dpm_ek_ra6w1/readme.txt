...
1.Project Overview
This MQTT example application demonstrates an MQTT client implementation on the RA6W1 device with DPM support . 
...
File Path:- wifi_mqtt_client_dpm_ek_ra6w1\e2studio\src\mqtt.h
...
8. Verifying Operation:
	...
	9. Console prints can be seen in Tera Term.
       ...
       11. Configure the MQTT broker and subscribe to the configured topic. The RA6W1 device wakes up at specified intervals from Deep Power Management (DPM) mode, publishes an MQTT message, and returns to DPM mode. The device also receives subscribed messages while operating in DPM mode
