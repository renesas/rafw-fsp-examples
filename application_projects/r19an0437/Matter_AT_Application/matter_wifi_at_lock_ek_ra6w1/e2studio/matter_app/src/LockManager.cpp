...
using namespace RenesDoorLock::LockInitParams;

constexpr EndpointId kLockEndpoint = 1;
...
CHIP_ERROR LockManager::Init(chip::app::DataModel::Nullable<chip::app::Clusters::DoorLock::DlLockState> state, LockParam lockParam)
{
    ...
    {
        RENES_LOG("Max number of users is greater than %d, the maximum amount of users currently supported on this platform",
                  kMaxUsers);
    ...
    {
        RENES_LOG("Max number of credentials per user is greater than %d, the maximum amount of users currently supported on this "
                  "platform",
    ...
    {
        RENES_LOG("Max number of schedules is greater than %d, the maximum amount of schedules currently supported on this platform",
    ...
    if (sLockTimer == NULL)
    {
        RENES_LOG("sLockTimer timer create failed");
...
bool LockManager::ReadConfigValues()
{
    ...
    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_LockUser, reinterpret_cast<uint8_t *>(&mLockUsers),
                                 sizeof(EmberAfPluginDoorLockUserInfo) * ArraySize(mLockUsers), outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_Credential, reinterpret_cast<uint8_t *>(&mLockCredentials),
                                 sizeof(EmberAfPluginDoorLockCredentialInfo) * ArraySize(mLockCredentials), outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_LockUserName, reinterpret_cast<uint8_t *>(mUserNames),
                                 sizeof(mUserNames), outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_CredentialData, reinterpret_cast<uint8_t *>(mCredentialData),
                                 sizeof(mCredentialData), outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_UserCredentials, reinterpret_cast<uint8_t *>(mCredentials),
                                 sizeof(CredentialStruct) *LockParams.numberOfUsers *LockParams.numberOfCredentialsPerUser,
                                 outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_WeekDaySchedules, reinterpret_cast<uint8_t *>(mWeekdaySchedule),
                                 sizeof(EmberAfPluginDoorLockWeekDaySchedule) *LockParams.numberOfWeekdaySchedulesPerUser *LockParams.numberOfUsers,
                                 outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_YearDaySchedules, reinterpret_cast<uint8_t *>(mYeardaySchedule),
                                 sizeof(EmberAfPluginDoorLockYearDaySchedule) *LockParams.numberOfYeardaySchedulesPerUser *LockParams.numberOfUsers,
                                 outLen);

    RnConfig::ReadConfigValueBin(RnConfig::kConfigKey_HolidaySchedules, reinterpret_cast<uint8_t *>(&(mHolidaySchedule)),
...
void LockManager::StartTimer(uint32_t aTimeoutMs)
{
    ...
    {
        RENES_LOG("sLockTimer timer start() failed");
...
void LockManager::CancelTimer(void)
{
    ...
    {
        RENES_LOG("sLockTimer stop() failed");
                         ...
                         const Nullable<chip::NodeId> &nodeId, const Optional<chip::ByteSpan> &pin, OperationErrorEnum &err)
{
    ...
    {
        setLockState(endpointId, fabricIdx, nodeId, DlLockState::kUnlatched, pin, err);
    }
                          ...
                          CredentialRuleEnum credentialRule, const CredentialStruct * credentials, size_t totalCredentials)
{
    ...
    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_LockUser, reinterpret_cast<const uint8_t *>(&mLockUsers),
                                  sizeof(EmberAfPluginDoorLockUserInfo) *LockParams.numberOfUsers);

    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_UserCredentials, reinterpret_cast<const uint8_t *>(mCredentials),
                                  sizeof(CredentialStruct) *LockParams.numberOfUsers *LockParams.numberOfCredentialsPerUser);

    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_LockUserName, reinterpret_cast<const uint8_t *>(mUserNames),
                                  sizeof(mUserNames));
                                ...
                                const chip::ByteSpan & credentialData)
{
    ...
    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_Credential, reinterpret_cast<const uint8_t *>(&mLockCredentials),
                                  sizeof(EmberAfPluginDoorLockCredentialInfo) *LockParams.numberOfCredentialsPerUser);

    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_CredentialData, reinterpret_cast<const uint8_t *>(&mCredentialData),
                                         ...
                                         uint8_t endHour, uint8_t endMinute)
{
    ...
    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_WeekDaySchedules, reinterpret_cast<const uint8_t *>(mWeekdaySchedule),
                                  sizeof(EmberAfPluginDoorLockWeekDaySchedule) *LockParams.numberOfWeekdaySchedulesPerUser *LockParams.numberOfUsers);
                                         ...
                                         DlScheduleStatus status, uint32_t localStartTime, uint32_t localEndTime)
{
    ...
    /*Save schedule information in NVM flash*/
    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_YearDaySchedules, reinterpret_cast<const uint8_t *>(mYeardaySchedule),
                                  sizeof(EmberAfPluginDoorLockYearDaySchedule) *LockParams.numberOfYeardaySchedulesPerUser *LockParams.numberOfUsers);
                                         ...
                                         uint32_t localStartTime, uint32_t localEndTime, OperatingModeEnum operatingMode)
{
    ...
    /*Save schedule information in NVM flash*/
    RnConfig::WriteConfigValueBin(RnConfig::kConfigKey_HolidaySchedules,
...
void LockManager::PostAttributeChange(EndpointId endpoint, AttributeId attributeId)
{
    AppEvent event;
