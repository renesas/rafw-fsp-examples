...
* Description  : Contains variables and functions used in new_thread0_entry.c.
**********************************************************************************************************************/
/***********************************************************************************************************************
...
static void load_wifi_profile(map_persistant_w_instance_ctrl_t *p_ctrl,WIFINetworkParamsExt_t *params)
{
    ...
    if (RM_MAP_PERSISTANT_W_Read_INT(p_ctrl, ENV_GROUP_WIFIPROFILE, WIFI_PROFILE_SYS_MODE, &result) != FSP_SUCCESS)
    {
    ...
    if (RM_MAP_PERSISTANT_W_Read_INT(p_ctrl, ENV_GROUP_WIFIPROFILE, WIFI_PROFILE_BAND, &result) != FSP_SUCCESS)
    {
    ...
    if (RM_MAP_PERSISTANT_W_Read_INT(p_ctrl, ENV_GROUP_WIFIPROFILE, WIFI_PROFILE_CHANNEL,&result) != FSP_SUCCESS)
    {
