...
* Description  : Contains variables and functions used in provisioning.c.
...
{
    if (tok->type == JSMN_STRING && strlen(s) == (size_t)(tok->end - tok->start) && strncmp(json + tok->start, s, tok->end - tok->start) == 0)
    {
        return 0;
    }
        return -1;
...
static char *json_network_add(char *mem, uint32_t mem_len, const char *ssid, security_type_t type, int32_t strength, int32_t isLast)
{
    int32_t len = 0;
...
static st_ble_wifiprovisionings_wifiprovisoning_t *json_network_info_response(WIFI_STATUS_t rc)
{
    static st_ble_wifiprovisionings_wifiprovisoning_t buf[32];

    snprintf((char *__restrict)buf, sizeof(buf), "{"
             "\"result\":%ld"
             "}", rc);
...
static st_ble_wifiprovisionings_wifiprovisoning_t *json_select_ap_response(WIFI_STATUS_t rc, const char *ssid, const char *passwd, security_type_t type)
{
    static st_ble_wifiprovisionings_wifiprovisoning_t buf[128];
...
static st_ble_wifiprovisionings_wifiprovisoning_t *json_check_network_response(WIFI_STATUS_t rc, const char *ssid, const char*passwd, security_type_t type)
{
    static st_ble_wifiprovisionings_wifiprovisoning_t buf[128];

    sprintf((char *__restrict)buf, (char *__restrict)sizeof(buf), "{"
             "\"result\":%ld,"
             "\"ssid\":\"%s\","
             "\"password\":\"%s\","
             "\"security\":%u"
...
static void params_cmd_factory_reset_print()
{
    LOG_INFO("prov_cmd_factory_reset_t: {}\n");
}

static void params_cmd_check_network_print()
{
    LOG_INFO("prov_cmd_check_network_t: {}\n");
}

static void params_cmd_reboot_print()
{
...
static void params_cmd_disconnect_print()
{
    LOG_INFO("prov_cmd_disconnect_t: {}\n");
}

static void params_cmd_wifi_status_print()
{
...
static void network_info(prov_cmd_network_info_t *params)
{
    ...
    uint16_t status = WIFI_CMD_ACK;

    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(json_network_info_response(WIFI_PROV_NETWORK_INFO));
...
static void select_ap(prov_cmd_select_ap_t *params)
{
    ...
    uint16_t notify_status = WIFI_CMD_ACK;
    WIFI_STATUS_t status = WIFI_CMD_SELECT_AP_SUCCESS;

    provisioning_cb.wifi_connect(params);

    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(json_select_ap_response(status, params->ssid, params->password, params->security_type));
...
static void scan()
{
	uint16_t status = WIFI_CMD_SCAN_AP_FAIL;
...
static void check_network()
{
    ...
    R_BLE_WIFIPROVISIONINGS_SetWifiprovisoning(json_check_network_response(status, provisioning_data.ssid, provisioning_data.password, provisioning_data.security_type));

    R_BLE_WIFIPROVISIONINGS_NotifyWifistatus(0, &notify_status);
}

static void network_info_event_handler(const char *cmd, jsmntok_t *token, uint32_t num_of_elements)
{
    ...
    for (uint32_t i = 0; i < num_of_elements; i++)
    {
        if (jsoneq(cmd, &token[i], "ping_addr") == 0)
        {
            ...
            int len = token[i+1].end - token[i+1].start;
        ...
        else if (jsoneq(cmd, &token[i], "svr_port") == 0)
        {
            ...
            {
            	params.srvr_port = (long unsigned int)value;
            }
        }
...
static void select_ap_event_handler(const char *cmd, jsmntok_t *token, uint32_t num_of_elements)
{
    ...
    for (uint32_t i = 0; i < num_of_elements; i++)
    {
        if (jsoneq(cmd, &token[i], "SSID") == 0)
        {
            /* String */
            const char *cmd_str_start = cmd + token[i+1].start;
            uint32_t len = (long unsigned int)(token[i+1].end - token[i+1].start);

            strncpy(params.ssid, cmd_str_start, MIN(sizeof(params.ssid), len));
        }
        else if (jsoneq(cmd, &token[i], "security_type") == 0)
        {
            ...
            {
                params.security_type = (long unsigned int)value;
            }
        }
        else if (jsoneq(cmd, &token[i], "password") == 0)
        {
            ...
            strncpy(params.password, cmd_str_start, MIN(sizeof(params.password), len));
        }
        else if (jsoneq(cmd, &token[i], "isHidden") == 0)
        {
            ...
            {
                params.is_hidden = (long unsigned int)value;
            }
        }
    }
...
{
    WIFI_STATUS_t status = WIFI_CMD_UNKNOWN_RCV;
    ...
    printf("rc is %d\n",(int)rc);
    printf("cmd: %s\n", cmd);
    printf("num_of_elements: %lu\n",(long unsigned int) num_of_elements);
    printf("  type:  %d\n", token->type);
    printf("  start: %d\n", token->start);
    printf("  end:   %d\n", token->end);
    printf("  size:  %d\n", token->size);
...
void provisioning_cmd_dispatch(char *cmd)
{
    ...
    for (uint32_t i = 0; i < (long unsigned int)num_of_elements; i++)
    {
