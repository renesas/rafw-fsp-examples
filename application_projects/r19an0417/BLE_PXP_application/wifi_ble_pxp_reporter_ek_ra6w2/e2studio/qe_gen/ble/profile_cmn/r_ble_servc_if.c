...
#include "discovery/r_ble_disc.h"
 ...
 #define QE_BLE_BYTE_POOL_LEN    0x800
#endif
...
#if (BSP_CFG_RTOS == 1)                /* ThreadX */
static TX_BYTE_POOL g_qe_byte_pool;
 #if defined(__GNUC__)
static uint8_t g_qe_byte_pool_buff[QE_BLE_BYTE_POOL_LEN] __attribute__((aligned(4)));
 #else
static uint8_t g_qe_byte_pool_buff[QE_BLE_BYTE_POOL_LEN];
 #endif
#endif
...
static void * qe_ble_malloc(uint32_t size);
static void   qe_ble_free(void * p_mem);
    ...
    if (NULL != p_attr)
    {
        ...
        if (attr_hdl == p_attr->p_attr_hdls[conn_idx].start_hdl + 1)
        {
            if (BLE_SUCCESS == result)
            {
                ...
                p_app_value = qe_ble_malloc(p_attr->app_size);
                ...
                else
                {
                    ...
                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_READ_RSP),
                                 BLE_SUCCESS,
                                 &evt_data);
                }

                qe_ble_free(p_app_value);
            }
            else
            {
                if (NULL != p_attr->read_rsp_cb)
            ...
            for (uint8_t d = 0; d < p_attr->num_of_descs; d++)
            {
                if (attr_hdl == p_attr->pp_descs[d]->p_attr_hdls[conn_idx])
                {
                    if (BLE_SUCCESS == result)
                    {
                        ...
                        p_app_value = qe_ble_malloc(p_attr->pp_descs[d]->app_size);
                        ...
                        else
                        {
                            ...
                            p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->pp_descs[d]->desc_idx, p_attr->inst_idx,
                                                                    BLE_SERVC_READ_RSP),
                                         BLE_SUCCESS,
                                         &evt_data);
                        }
    ...
    if (NULL != p_attr)
    {
        ...
        if (attr_hdl == p_attr->p_attr_hdls[conn_idx].start_hdl + 1)
        {
            ...
            else
            {
                ...
                p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_WRITE_RSP),
                             result,
                             &evt_data);
            }
        }
...
void R_BLE_SERVC_GattcCb (uint16_t type, ble_status_t result, st_ble_gattc_evt_data_t * p_data)
{
    switch (type)
    {
            ...
            if (NULL != p_attr)
            {
                    ...
                    p_attr->p_attr_hdls[conn_idx].start_hdl + 1)
                {
                    if (BLE_SUCCESS == result)
                    {
                        ...
                        {
                            gs_conn_info[conn_idx].p_read_buf = qe_ble_malloc(p_attr->db_size);
                        ...
                        gs_conn_info[conn_idx].read_buf_pos =
                            (uint16_t) (gs_conn_info[conn_idx].read_buf_pos +
                                        p_rd_char_evt_param->read_data.value.value_len);
                    }
                    else
                    {
                        if (NULL != gs_conn_info[conn_idx].p_read_buf)
                        {
                            qe_ble_free(gs_conn_info[conn_idx].p_read_buf);
                        ...
                        else
                        {
                            ...
                            p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx,
                                                                    BLE_SERVC_READ_RSP),
                                         result,
                                         &evt_data);
                        }
                    ...
                    for (uint8_t d = 0; d < p_attr->num_of_descs; d++)
                    {
                            ...
                            p_attr->pp_descs[d]->p_attr_hdls[conn_idx])
                        {
                            if (BLE_SUCCESS == result)
                            {
                                if (NULL == gs_conn_info[conn_idx].p_read_buf)
                                {
                                    gs_conn_info[conn_idx].p_read_buf = qe_ble_malloc(p_attr->pp_descs[d]->db_size);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->pp_descs[d]->desc_idx,
                                                                            p_attr->pp_descs[d]->inst_idx,
                                                                            BLE_SERVC_READ_RSP),
                                                 result,
                                                 &evt_data);
                                }
        ...
        case BLE_GATTC_EVENT_LONG_CHAR_READ_COMP:
        {
            ...
            read_evt_handler(p_data->conn_hdl, gs_conn_info[conn_idx].read_attr_hdl, &value, result);
        ...
        case BLE_GATTC_EVENT_HDL_VAL_NTF:
        {
            ...
            if ((NULL != p_attr) && (NULL != p_attr->decode))
            {
                ...
                else
                {
                    ...
                    p_client->cb(BLE_SERVC_MULTI_ATTR_EVENT(p_attr->char_idx, p_attr->inst_idx, BLE_SERVC_HDL_VAL_NTF),
                                 BLE_SUCCESS,
                                 &evt_data);
                }
...
#if (BSP_CFG_RTOS == 1)                /* ThreadX */
    UINT status = tx_byte_pool_create(&g_qe_byte_pool, "QE_SERVC_HEAP", &g_qe_byte_pool_buff[0], QE_BLE_BYTE_POOL_LEN);
...
ble_status_t R_BLE_SERVC_WriteChar (const st_ble_servc_char_info_t * p_attr, uint16_t conn_hdl,
                                    const void * p_app_value)
{
    ...
    p_byte_value = qe_ble_malloc(p_attr->db_size);
...
ble_status_t R_BLE_SERVC_WriteDesc (const st_ble_servc_desc_info_t * p_attr, uint16_t conn_hdl,
                                    const void * p_app_value)
{
    ble_status_t ret;
                                              ...
                                              const st_ble_seq_data_t        * p_app_value)
{
    ...
    p_byte_value = qe_ble_malloc(write_len);
                             ...
                             void                      * p_param)
{
    ...
    switch (type)
    {
        case BLE_DISC_PRIM_SERV_FOUND:
        {
            ...
            memcpy(&p_info->p_attr_hdls[conn_idx], &p_serv_param->value.serv_16.range,
                   sizeof(p_info->p_attr_hdls[conn_idx]));
            break;
        }
            ...
            for (uint8_t c = 0; c < p_info->num_of_chars; c++)
            {
                    ...
                    (BLE_GATT_INVALID_ATTR_HDL_VAL == p_info->pp_chars[c]->p_attr_hdls[conn_idx].start_hdl))
                {
                    ...
                    for (uint8_t d = 0; d < p_char_param->num_of_descs; d++)
                    {
                        ...
                        {
                            if ((p_info->pp_chars[c]->pp_descs[dd]->uuid_16 ==
                                 p_char_param->descs[d].value.desc_16.uuid_16) &&
                                (BLE_GATT_INVALID_ATTR_HDL_VAL ==
                                 p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx]))
                            {
                                p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx] =
                                    p_char_param->descs[d].value.desc_16.desc_hdl;
                                break;
                            }
                            else if ((0 ==
                                      memcmp(p_info->pp_chars[c]->pp_descs[dd]->uuid_128,
                                             p_char_param->descs[d].value.desc_128.uuid_128, 0x10)) &&
                                     (BLE_GATT_INVALID_ATTR_HDL_VAL ==
                                      p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx]))
                            {
                                p_info->pp_chars[c]->pp_descs[dd]->p_attr_hdls[conn_idx] =
                                    p_char_param->descs[d].value.desc_128.desc_hdl;
                                break;
                            }
                        }
                    }
...
static void * qe_ble_malloc (uint32_t size)
{
    ...
    status = tx_byte_allocate(&g_qe_byte_pool, (VOID **) &p_mem, size, TX_NO_WAIT);
    if (TX_SUCCESS != status)
    {
        p_mem = NULL;
    }
