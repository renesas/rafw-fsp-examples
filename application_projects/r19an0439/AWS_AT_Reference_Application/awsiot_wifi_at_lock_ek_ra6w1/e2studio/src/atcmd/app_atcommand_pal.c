...
{
#if defined(__SUPPORT_ATCMD__)
    char *broker_nvram = NULL;

    RM_MAP_PERSISTANT_W_Read_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_APPCFG,
                                    AWSIOT_CFG_BROKER_URL, &broker_nvram);
    if (broker_nvram)
    {
        printf("AWS Broker URL from NVRAM: %s\n\r", broker_nvram);
        return broker_nvram;
    }
    else
    {
        printf("No AWS Broker URL found in NVRAM. Using default AWS Broker URL: %s\n\r", broker);
        return broker;
    }
#else
    return broker;
...
uint32_t pal_app_get_broker_port(uint32_t port)
{
    ...
    int broker_port = 0;
    RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_APPCFG,
                                 AWSIOT_CFG_PORT, (int*)&broker_port);
    if (broker_port > 0)
    {
        printf("AWS MQTT Port from NVRAM: %d\n\r", broker_port);
        return broker_port;
    }
    else
    {
        printf("No AWS MQTT Port found in NVRAM. Using default Port: %d\n\r", (int)port);
        return port;
    }
#else
    return port;
