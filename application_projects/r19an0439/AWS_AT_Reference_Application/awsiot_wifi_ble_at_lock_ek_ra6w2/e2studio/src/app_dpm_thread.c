...
static void dpmAPPManager(void *_threadInfo)
{
    ...
    else
    {
        ...
        if (RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_APPCFG,
                                         AWSIOT_CFG_USE_DPM, (int*)&use_dpm_nvram) != 0)
        {
            APRINTF_I("\nFailed to read the flag usedpm from NVRAM appcfg or usedpm flag is not set.\n\n");
        }
        else
        {
            use_dpm_nvram = use_dpm_nvram & 0x000000FF;
            APRINTF_I("\n\nNVRAM Value usedpm masked: %d\n\n", use_dpm_nvram);
        }

        if ((threadInfo->DpmMode == 1) && (use_dpm_nvram == 1))
        {
