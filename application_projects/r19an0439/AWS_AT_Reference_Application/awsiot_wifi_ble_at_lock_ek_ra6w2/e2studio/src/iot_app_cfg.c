    ...
    if (isSntpNeed == 1)
    {
        ...
        if (use == 0)
        {
            ...
            RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_SYSCFG,
                                          NVR_KEY_SNTP_SYNC_PERIOD, DFLT_SNTP_SYNC_PERIOD);
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_SYSCFG,
                                             NVR_KEY_SNTP_SERVER_DOMAIN, DFLT_SNTP_SERVER_DOMAIN);
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_SYSCFG,
                                             NVR_KEY_SNTP_SERVER_DOMAIN_1, DFLT_SNTP_SERVER_DOMAIN_1);
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_SYSCFG,
                                             NVR_KEY_SNTP_SERVER_DOMAIN_2, DFLT_SNTP_SERVER_DOMAIN_2);
#else
            write_nvram_syscfg_int(NVR_KEY_SNTP_SYNC_PERIOD, DFLT_SNTP_SYNC_PERIOD);
