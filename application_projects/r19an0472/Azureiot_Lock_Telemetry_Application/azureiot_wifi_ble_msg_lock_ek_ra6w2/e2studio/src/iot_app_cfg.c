...
* Description  : IoT application configuration and initialization functions.
***********************************************************************************************************************/
/***********************************************************************************************************************
...
char* app_cfg_get_ip_from_fast_dns(char *hostname, UINT8 is_real_query)
{
    ...
    strncpy(ip_str, app_cfg_get_server_ip(), sizeof(ip_str) - 1);
...
void app_common_set_time_sync_from_normal_sntp(UINT32 _sntpCount, UINT8 _sleepMode, UINT32 _sleepInterval)
{
    ...
    if (RM_PMGR_W_dpm_is_wakeup() || (wakeupSource == WAKEUP_COUNTER_WITH_RETENTION && wakeupType == 0 /*DPM_UNKNOWN_WAKEUP*/)
    ...
    while (1) {
        if (flagSntp || is_sntp_sync()) {
            ...
            if (loop_cnt++ > _sntpCount) {
                ...
                {
                    //app_common_goto_sleep_1_or_2(SLEEP_MODE_2, 1, _sleepInterval, saveTime);
    ...
    unsigned long long added_sleep_dur)
{
    ...
    if (wakeupSource == WAKEUP_SOURCE_WAKEUP_COUNTER || wakeupSource == WAKEUP_SOURCE_EXT_SIGNAL
        || wakeupSource == WAKEUP_EXT_SIG_WITH_RETENTION /*|| wakeupSource == WAKEUP_SENSOR_EXT_SIGNAL*//*[azure dpm freertos work]*/) {
        readSntpTime = read_sntp_time_from_nvram();
        rcode = RM_MAP_PERSISTANT_W_Read_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_APPCFG,
                                             AWSIOT_CFG_SNTP_SUCCESS_FLAG, (int *)&isSntpSuccess);//[azure dpm freertos work]
        ...
        if (readSntpTime != 0 && rcode == FSP_SUCCESS && isSntpSuccess == 1) {
            ...
            ra6w1_time64(&setTime, &now); /* set UTC Time *///[azure dpm freertos work]
    ...
    while (1) {
        if (is_sntp_sync()) {
            ...
            ra6w1_time64(NULL, &now);//[azure dpm freertos work]
            save_sntp_time_nvram((unsigned long)now);

            printf("[Saved SNTP Time in NVRAM] : %llu \n", now);

            //ts = (struct tm*)da16x_localtime64(&now);
            ts = (struct tm*)ra6w1_localtime64(&now);//[azure dpm freertos work]

            //da16x_strftime(buf, sizeof(buf), "%Y.%m.%d %H:%M:%S", ts);
            ra6w1_strftime(buf, sizeof(buf), "%Y.%m.%d %H:%M:%S", ts);//[azure dpm freertos work]
            printf("SNTP Sync: %s : checked\n", buf);

            isSntpSuccess = 1;
            if (RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_APPCFG,
                                              AWSIOT_CFG_SNTP_SUCCESS_FLAG, isSntpSuccess))//[azure dpm freertos work]
                printf("%s (=%d) on NVRAM write failed\n", NV_SNTP_SUCCESS_FLAG, isSntpSuccess);
...
void *persist_storage_read(property_id_t id)
{
    switch (id) {
    case PROP_SVR_IP:
...
int persist_storage_write(property_id_t id, const void *value)
{
    switch (id) {
    case PROP_SVR_IP:
