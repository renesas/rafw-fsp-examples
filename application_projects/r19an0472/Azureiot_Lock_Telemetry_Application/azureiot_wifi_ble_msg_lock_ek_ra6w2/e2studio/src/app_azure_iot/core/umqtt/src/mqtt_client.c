...
//azurefrtoswork[[: for compile with this SDK
...
//AZURE_CHANGES[[::
//azuredpmwork[[: set packetSendTimeMs after open completed
				if (RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected())
				{
//azure]]
					app_print_elapse_time_ms("[%s:%d] sendPacketItem() bypassed (size=%d)", __func__, __LINE__, size);
					(void)tickcounter_get_current_ms(mqtt_client->packetTickCntr, &mqtt_client->packetSendTimeMs);
				}
				else
				{
//AZURE_CHANGES]]
...
//]]
        ...
        {
//azuredpmwork[[: set reconnection flag & set rcv timeout to initial value
#if CFG_PMGR
...
//azurefrtoswork[[: debug
			LogError("mqtt_codec_bytesReceived() in size = %d", size);
//]]
...
//azurefrtoswork[[: debug
        LogError("MQTT_CLIENT_CONNECTION_ERROR: mqtt_client=%p", mqtt_client);
//]]
...
//azure[[:: mqttOptions->keepAliveInterval is coming as 0, so assigning AZURE_CONFIG_KEEP_INTERVAL_DEF.
        mqtt_client->mqttOptions.keepAliveInterval = AZURE_CONFIG_KEEP_INTERVAL_DEF;
//azure]]
...
#endif
//azuredpmwork[[: save subscription info for DPM wakeup
						if (!RM_PMGR_W_dpm_is_wakeup() || app_is_reconnected())
                        //dpm_debug
						{
//azure]]
							app_dpm_set_subscribe_info(suback.packetId, suback.qosCount, (UINT32*)suback.qosReturn);
							/*
...
//]]
    ...
    if (mqtt_client != NULL && mqtt_client->xioHandle != NULL)
    {
        ...
        {
//azuredpmwork[[: force to send CONNACK to Azure S/W stack when DPM wakeup & socket connected
			if (m_status != mqtt_client->mqtt_status)
			{
				 LogDebug("mqtt_status = %d", mqtt_client->mqtt_status);
				m_status = mqtt_client->mqtt_status;
			}

			if (RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected() && mqtt_client->mqtt_status == MQTT_STATUS_SOCKET_CONNECTED)
			{
				CONNECT_ACK connack = { 0 };
