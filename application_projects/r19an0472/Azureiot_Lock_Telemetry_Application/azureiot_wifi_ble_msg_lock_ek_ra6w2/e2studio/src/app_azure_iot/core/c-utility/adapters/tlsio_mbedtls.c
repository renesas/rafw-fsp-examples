...
#include "rm_lwip_w_cfg.h"//AZURE_CHANGES
...
static int decode_ssl_received_bytes(TLS_IO_INSTANCE *tls_io_instance)
{
    ...
    while (rcv_bytes > 0)
    {
		...
		if((rcv_bytes >= 0) || (rcv_bytes == MBEDTLS_ERR_SSL_TIMEOUT))
		{
			IoT_Error_t status = 0;
			status = azure_tls_save(AZURE_TLS_SOCK_NAME, &tls_io_instance->ssl);
			if (status)
			{
...
//azuredpmwork[[: bypss handshake step if saved TLS state is MBEDTLS_SSL_HANDSHAKE_OVER
			app_dpm_set_recv_timeout_flag(DPM_RCV_READY_HANDSHAKE);
			if (tls_io_instance->ssl.MBEDTLS_PRIVATE( state ) == MBEDTLS_SSL_HANDSHAKE_OVER)
			{
				LogInfo("Already TLS session established");
				result = 0;
				goto BYPASS_HANDSHAKE;
			}
//]]
...
//azuredpmwork[[: save TLS session & set rcv time to DPM_RCV_OK_CONNECT
				IoT_Error_t status = 0;
...
//azuredpmwork[[: workaround for UC wakeup crash
		mbedtls_ssl_set_sync_iv(&tls_io_instance->config, pdTRUE, 10);	
//]
        mbedtls_ssl_setup(&tls_io_instance->ssl, &tls_io_instance->config);
//azuredpmwork[[: restore TLS session when DPM wakeup
		if(RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected())
		{				
			IoT_Error_t dpm_status = azure_tls_restore(AZURE_TLS_SOCK_NAME, &tls_io_instance->ssl);
	
			if (dpm_status != SUCCESS)
			{
				LogError("azure_tls_restore() fail");
			}
		}
//]]
...
void tlsio_mbedtls_destroy(CONCRETE_IO_HANDLE tls_io)
{
    if (tls_io != NULL)
    {
        ...
        mbedtls_uninit(tls_io_instance);
//azuredpmwork[[: clear TLS session on DPM mode
#if CFG_PMGR
		if(RM_PMGR_W_dpm_is_enabled())
#endif
		{				
			IoT_Error_t dpm_status = azure_tls_clear(AZURE_TLS_SOCK_NAME);
	
			if (dpm_status != SUCCESS)
			{
				LogError("azure_tls_clear() fail");
			}
		}
//]]
...
//azuredpmwork[[: reset TLS session in not case of DPM wakeup
			if (!RM_PMGR_W_dpm_is_wakeup() || app_is_reconnected())
			{
//azure]]
...
//azuredpmwork[[: save TLS session when read operation completed
