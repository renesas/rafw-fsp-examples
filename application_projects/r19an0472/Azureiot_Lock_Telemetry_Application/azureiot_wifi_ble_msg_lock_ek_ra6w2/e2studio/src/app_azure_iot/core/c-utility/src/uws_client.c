...
//azuredpmwork[[: bypass xio_send and WS_OPEN_OK on DPM mode, ToDo - how process WS_OPEN_ERROR_CANNOT_SEND_UPGRADE_REQUEST on DPM mode
							if (RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected())
							{
//azure]]
								LogDebug("xio_send() bypassed (size=%d)", upgrade_request_length);
