...
static OPTIONHANDLER_HANDLE CreateInternal(pfCloneOption cloneOption, pfDestroyOption destroyOption, pfSetOption setOption)
{
    ...
    else
    {
        ...
        result->storage = VECTOR_create(sizeof(OPTION_t));//AZURE_CHANGES
...
static void DestroyInternal(OPTIONHANDLER_HANDLE handle)
{
    if (handle != NULL)
    {
        ...
        {
            OPTION_t* option = (OPTION_t*)VECTOR_element(handle->storage, i);//AZURE_CHANGES
            if (option != NULL)
            {
...
OPTIONHANDLER_HANDLE OptionHandler_Clone(OPTIONHANDLER_HANDLE handler)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                OPTION_t* option = (OPTION_t*)VECTOR_element(handler->storage, i);//AZURE_CHANGES
                if (option != NULL)
                {
