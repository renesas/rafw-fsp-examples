...
//#include <net/if.h> //AZURE_CHANGES
...
//azuredpmwork[[: reboot logic when connection failed on wakeup mode
#define MAX_RECONN_COUNT_FOR_REBOOT		1
//]]

//azurefrtoswork[[: API for setting rcv timeout added
static void app_socket_set_recv_timeout(int32_t socket, uint32_t _timeMs)
{
	...
	if (timeOut != _timeMs)
	{
		...
		sockopt_timeout.tv_usec = _timeMs * 1000;

		ret = setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, &sockopt_timeout, sizeof(sockopt_timeout));
		if (ret != 0) {
			LogError("Failed to set socket option - SO_RCVTIMEOUT(%d:%d)", ret, errno);
		}
		else
		{
			LogInfo("recv timeout(=%d) set OK (socket=%d)", _timeMs, socket);
			timeOut = _timeMs;
		}
	}
}
...
{
	LogError("this API not supported");
	return MU_FAILURE;
}
...
static int initiate_socket_connection(SOCKET_IO_INSTANCE* socket_io_instance)
{
   ...
   // struct sockaddr_un addrInfoUn; //AZURE_CHANGES azurefrtoswork
    ...
    {
    	LogError ("[%s:%d] socket_io_instance->address_type(=%d) not supported", __func__, __LINE__, socket_io_instance->address_type);
		result = MU_FAILURE;
    }
...
//azuredpmwork[[: set timeout & bind for DPM wakeup added
#if CFG_PMGR
			int ret = 0;

			if (!RM_PMGR_W_dpm_is_wakeup() || app_is_reconnected())
			{
				app_socket_set_recv_timeout(socket_io_instance->socket, 2000);
			}
			...
			local_addr.sin_port = htons(app_socket_get_local_port());
			ret = bind(socket_io_instance->socket, (struct sockaddr *)&local_addr, sizeof(struct sockaddr_in));
			if (ret != 0) {
				LogError ("Failed to bind socket of tcp client(%d:%d)", ret, errno);
				result = MU_FAILURE;
			}
			else
			{
				 LogInfo("bind OK (socket=%d)", socket_io_instance->socket);
			}
#endif

			flags = fcntl(socket_io_instance->socket, F_GETFL, 0);
...
//]]
		...
		result = connect(socket_io_instance->socket, addr->ai_addr, addr->ai_addrlen);
			...
			if(getSleepMode() == SLEEP_MODE_3){
				...
				app_common_goto_sleep_1_or_2(SLEEP_MODE_2, getSleepRTMMem(), getUsecTimer(), 1);
			}
		...
		else
		{
			...
			app_print_elapse_time_ms("[%s:%d] Connection OK (socket=%d)", __func__, __LINE__, socket_io_instance->socket);
			result = 0;
...
static int wait_for_socket_connection(SOCKET_IO_INSTANCE* socket_io_instance)
{
 int result;
	...
	rc = getpeername(socket_io_instance->socket, (struct sockaddr *)&peer_addr, (socklen_t *)&addr_len);
	if (rc < 0)
	{
		LogError("socket(=%d) not connected to peer", socket_io_instance->socket);
		result = MU_FAILURE;
	}
	else
	{
		char buf_ip[16] = {0,};
		longtoip(ntohl(peer_addr.sin_addr.s_addr), buf_ip);
		app_print_elapse_time_ms("socket(=%d) connected to peer (srv IP=\"%s\", port=%d)", socket_io_instance->socket, buf_ip, ntohs(peer_addr.sin_port));
		result = 0;
	}
...
//AZURE_CHANGES azurefrtoswork[[::
			...
			if (socket_io_instance->address_type != ADDRESS_TYPE_IP)
			{
				LogError ("socket_io_instance->address_type(=%d) not supported", socket_io_instance->address_type);
			}
			if (app_is_reconnected())

			{
				app_socket_set_port_n_filter(app_dpm_get_client_socket_port());				
			}

			socket_io_instance->socket = socket_dpm (AZURE_TCP_SOCK_NAME, AF_INET, SOCK_STREAM, 0);
			if (socket_io_instance->socket >= 0)
			{
				LogDebug ("Socket(=%d) Creation OK", socket_io_instance->socket);
			}
#endif			
            if (socket_io_instance->socket < SOCKET_SUCCESS)
            {
    ...
    if (socket_io != NULL)
    {
        ...
        if (socket_io_instance->io_state == IO_STATE_OPEN)
        {
			...
			if (socket_io_instance->io_state == IO_STATE_OPEN)
			{
				...
				if (app_dpm_get_recv_timeout_flag())
				{				
					...
					else if(app_dpm_get_recv_timeout_flag() == DPM_RCV_READY_HANDSHAKE)
					{						
						time_out = 2000;
					}
					else
				...
				app_socket_set_recv_timeout(socket_io_instance->socket, time_out);

				if (RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected() && !app_dpm_is_subscribed())
				{
					LogDebug("recv() bypassed til ready");
					return;
				}

				if (app_dpm_get_recv_timeout_flag() == DPM_RCV_READY_HANDSHAKE)
				{
					...
					{
						LogError("Socketio_Failure [Handshake]: Receiving data from endpoint: errno=%d.", errno);
						indicate_error(socket_io_instance);
					}
				...
				{		
					...
					{
						received = recv(socket_io_instance->socket, socket_io_instance->recv_bytes, XIO_RECEIVE_BUFFER_SIZE, MSG_NOSIGNAL);
						//azuredpmwork[[: exit DPM sleep state when received
...
//							if (is_sleep3_wakeup) {
								if (app_dpm_get_recv_timeout_flag() == DPM_RCV_OK_SLEEP)
								{
									//]]
									// printf("clear dpm sleep due to recv(len=%d, errno=%d)", received, errno);
								}
...
//							}
						...
						else if (received == 0)
						{
							...
							if (errno == ENOTCONN)
							{
								LogError("recv(fd=%d): NG -> errno = %d\n", socket_io_instance->socket, errno);

								indicate_error(socket_io_instance);
							}
							else
							{
								LogInfo("this is probably the socket being closed on the other end(errno=%d)", errno);

							}
						}
						else if (received < 0 && errno != EAGAIN && errno != EWOULDBLOCK)
						{
							LogError("Socketio_Failure: Receiving data from endpoint: errno=%d (socket=%d).", errno, socket_io_instance->socket);

							indicate_error(socket_io_instance);
						}

						if (received <= 0 && (errno == EAGAIN || errno == EWOULDBLOCK) && app_dpm_get_recv_timeout_flag() == DPM_RCV_OK_SLEEP)
						{
							if (RM_PMGR_W_dpm_is_enabled())

							{					
								goSleepAppDpmThread(1, DPM_RTC_NORMAL_MODE);
							}
						}
			...
			if (socket_io_instance->io_state == IO_STATE_OPENING)
			{
				...
				else
				{
					if(socket_io_instance->io_state == IO_STATE_OPEN)
					{
						...
						{
							LogError("Socketio_Failure: wait_for_socket_connection failed line = %d",__LINE__);
							indicate_error(socket_io_instance);
						}
					}
				}
