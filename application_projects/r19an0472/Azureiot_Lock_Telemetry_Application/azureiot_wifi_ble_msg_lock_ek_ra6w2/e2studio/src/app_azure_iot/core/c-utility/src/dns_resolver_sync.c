...
// in the project
//#include "socket_async_os.h" //AZURE_CHANGES
...
//AZURE_CHANGES azurefrtoswork[[::  
	    ...
	    //ms_result = mallocAndStrcpy_s(&result->hostname, hostname);
			if(app_dpm_get_skip_saving_serverip() == TRUE)
			{
				ipAddr = app_common_get_ip_from_noraml_dns(hostname);
			}
			else
			{
				if (getSleepMode() == SLEEP_MODE_1)
				{
					if (SLEEP_1_USE_RTM != 0)
					{
						ipAddr = app_common_get_ip_from_fast_dns_with_rtm((char *)hostname, AZURE_RTM_NAME, app_is_reconnected());
					}
					else
					{
						ipAddr = app_common_get_ip_from_fast_dns_with_nvram((char *)hostname, app_is_reconnected());
					}
				}
				else if (getSleepMode() == SLEEP_MODE_2)
				{
			...
			if(ipAddr == NULL)				
				...
				ms_result = mallocAndStrcpy_s(&result->hostname, ipAddr);
//AZURE_CHANGES azurefrtoswork]]			
            if (ms_result != 0)
            {
                /* Codes_SRS_dns_resolver_30_014: [ On any failure, dns_resolver_create shall log an error and return NULL. ]*/
