...
static int publishDeviceTwinGetMsg(MQTTTRANSPORT_HANDLE_DATA* transport_data, MQTT_DEVICE_TWIN_ITEM* mqtt_info)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
//azuredpmwork[[: 
            	 LogInfo ("publish reported data OK ");
//azuredpmwork]] 
                DList_InsertTailList(&transport_data->ack_waiting_queue, &mqtt_info->entry);
//azuredpmwork[[: set flag for publish report
//azure[[:
#if CFG_PMGR
...
/*
				...
				{
					if (transport_data->currPacketState == PUBLISH_TYPE && dpm_mode_is_enabled() && app_dpm_get_recv_timeout_flag() == DPM_RCV_OK_CONNECT)
					{
						app_dpm_set_recv_timeout_flag(DPM_RCV_OK_SLEEP);
					}
					else
...
//azure[[:: FIX for "Unexpected data size returned from  mbedtls_ssl_write"
    while (list_item && list_item != twin_list)
//azure]]
    {
...
//azure[[:
#if CFG_PMGR
		//azuredpmwork[[: test for defensing sleep issue

        if (RM_PMGR_W_dpm_is_wakeup() && app_dpm_get_recv_timeout_flag() == DPM_RCV_OK_CONNECT
			&& app_dpm_get_report_send_flag() && !app_is_reconnected())

		{
//azure]]
			LogInfo("ack_waiting_queue timeout & removed ");

			LogDebug("ack_waiting_queue timeout & removed ===> do sleep(pub ack = %d)", app_dpm_is_received_pub_ack());
			app_dpm_set_recv_timeout_flag(DPM_RCV_OK_SLEEP);
		}
//]]
#endif
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                const APP_PAYLOAD* payload = mqttmessage_getApplicationMsg(msgHandle);
                //dpm_debug
                // printf("\n\t************ payload->message = %s line %d***********\t\n", payload->message,__LINE__);
                if (payload == NULL)
                {
...
//azuredpmwork[[: will be received data after subscription completed
//azure[[:
#if CFG_PMGR
                    if (RM_PMGR_W_dpm_is_wakeup() && !app_is_reconnected())
					{
						//set flag to true for pendig publish on DPM mode
						if (transport_data->twin_resp_sub_recv == false)
						{
							transport_data->twin_resp_sub_recv = true;
						}
...
//azurefrtoswork[[: debug
				LogInfo("mqttClientStatus set to MQTT_CLIENT_STATUS_NOT_CONNECTED");
//]]
                break;
            }
...
static void DisconnectFromClient(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    ...
    {
//azuredpmwork[[: set reconnection flag & set timeout with initial value
#if CFG_PMGR
...
//]]
        ...
        transport_data->mqttClientStatus = MQTT_CLIENT_STATUS_NOT_CONNECTED;
//azurefrtoswork[[: debug
		LogDebug("mqttClientStatus set to MQTT_CLIENT_STATUS_NOT_CONNECTED");
//]]
        transport_data->currPacketState = DISCONNECT_TYPE;
    }
}
...
static void processErrorCallback(MQTT_CLIENT_HANDLE handle, MQTT_CLIENT_EVENT_ERROR error, void* callbackCtx)
{
    ...
    if (callbackCtx != NULL)
    {
        ...
        switch (error)
        {
            ...
            {
//azurefrtoswork[[: debug
                LogDebug("MQTT_CLIENT_COMMUNICATION_ERROR");
//]]
...
//azure[[:
#if CFG_PMGR
//azuredpmwork[[: bypass subscription when wakeup on DPM mode
	if (!RM_PMGR_W_dpm_is_wakeup() || app_is_reconnected())
			{
#endif
//azure]]
				...
				else
				{
					...
					{
						LogDebug("subscribed topics=0x%04x", transport_data->topics_ToSubscribe);
					}
				}
			}
			else
			{
				...
				app_print_elapse_time_ms("[%s:%d] mqtt_client_subscribe() bypassed", __func__, __LINE__);

				suback.qosReturn = (QOS_VALUE*)app_dpm_get_subscribe_info(&suback.packetId, (uint8_t*)&suback.qosCount);
				/* debug
				LogInfo("subscribe info: packetId=%d, qosCount=%d", suback.packetId, suback.qosCount);
				for (int i=0; i<suback.qosCount; i++)
				{
					LogInfo("subQoS[%d] = %d", i, suback.qosReturn[i]);
				}
				*/
				mqttOperationCompleteCallback(transport_data->mqttClient, MQTT_CLIENT_ON_SUBSCRIBE_ACK, (void*)&suback, transport_data);
        ...
        else
        {
            ...
            if (CreateTransportProviderIfNecessary(transport_data) == 0)
            {
                ...
                else
                {
                    ...
                    result = 0;
//azurefrtoswork[[: 
                    LogInfo("MQTT client connected OK (time(ms)=%llu)", transport_data->mqtt_connect_time);
//]]
                }
            }
            else
        ...
        if (transport_data->mqttClientStatus == MQTT_CLIENT_STATUS_NOT_CONNECTED && transport_data->isRecoverableError)
        {
            ...
            if (!transport_data->conn_attempted || retry_control_should_retry(transport_data->retry_control_handle, &retry_action) != 0 || retry_action == RETRY_ACTION_RETRY_NOW)
            {
                ...
                else
                {
                    ...
                    {
//azuredpmwork[[: bypass state on DPM mode
//azure[[:
                    	if (!RM_PMGR_W_dpm_is_wakeup() || app_is_reconnected())
						{
//azure]]
							transport_data->mqttClientStatus = MQTT_CLIENT_STATUS_CONNECTING;
							app_print_elapse_time_ms("[%s:%d] mqttClientStatus set to MQTT_CLIENT_STATUS_CONNECTING", __func__, __LINE__);
						}
						else
						{
							app_print_elapse_time_ms("[%s:%d] mqttClientStatus set to MQTT_CLIENT_STATUS_CONNECTING bypassed status: %d", __func__, __LINE__, transport_data->mqttClientStatus);
						}
//]]
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                    	...
                    	//azure[[:: CRASH FIX
                        memset(&(state->pending_get_twin_queue), 0, sizeof(DLIST_ENTRY));
                    	memset(&(state->ack_waiting_queue), 0, sizeof(DLIST_ENTRY));
                    	memset(&(state->telemetry_waitingForAck), 0, sizeof(DLIST_ENTRY));
                    	//azure]]
                        ...
                        state->keepAliveValue = app_get_ka_value();
