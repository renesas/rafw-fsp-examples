...
//azuredpmwork[[: DPM sleep point for no callback
				else
				{
					LogInfo(" PUB ACK status_code = %d, item_id = %d ==>", status_code, item_id);
#if CFG_PMGR
					if (RM_PMGR_W_dpm_is_enabled() && app_dpm_get_recv_timeout_flag() == DPM_RCV_OK_CONNECT)
#endif
					{
...
{
    // printf("\n***api_test***IoTHubClientCore_LL_SetOption****\n");
    IOTHUB_CLIENT_RESULT result;
    if (
        (iotHubClientHandle == NULL) ||
        (optionName == NULL) ||
