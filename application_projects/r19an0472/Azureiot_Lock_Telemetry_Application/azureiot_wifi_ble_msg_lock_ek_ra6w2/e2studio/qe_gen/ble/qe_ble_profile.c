...
const attribute_t qe_ble_profile[QE_ATTRIBUTE_HANDLE_PROFILE_END] =
{
    ...
    // Service Declaration: WiFiProvisioning
    [16] = {
        .handle                  = 16,
        .encapsulated_attributes = 14,
        .permissions             = 0x01,
        .uuid_length             = QE_BLE_PROFILE_UUID_SIZE_ADOPTED,
        .value_length            = 0x10,
	...
	// Characteristic Declaration: ThingName read
	    [28] = {
	        .handle                  = 28,
	        .encapsulated_attributes = 0,
	        .permissions             = 0x01,   // Readable declaration
	        .uuid_length             = QE_BLE_PROFILE_UUID_SIZE_ADOPTED,
	        .value_length            = 0x13,   // Decl length: props + handle + UUID128
	        .uuid                    = (uint8_t *) &qe_uuid2, // 0x2803
	        .value                   = (uint8_t []) {
	                                      0x02, 0x1C, 0x00,   // Properties = read (0x02), Value Handle = 28
	                                      0xC1, 0xF5, 0xDC, 0x5C, 0xB3, 0x47, 0xCA, 0xA3,
	                                      0x27, 0x47, 0x4B, 0x1B, 0x07, 0xB2, 0x61, 0x91  // Azure UUID (0X07…)
	                                   },
	        .notify_write            = 1,
	        .notify_read             = 0,
	        .read_indication         = 0,
    ...
    // Characteristic Value: ThingName read
		[29] = {
			.handle                  = 29,
			.encapsulated_attributes = 0,
			.permissions             = 0x01,  // Read permitted
			.uuid_length             = QE_BLE_PROFILE_UUID_SIZE_CUSTOM,
			.value_length            = 0xC0,  // buffer size for JSON input
			.uuid                    = (uint8_t []) {
										  0xC1, 0xF5, 0xDC, 0x5C, 0xB3, 0x47, 0xCA, 0xA3,
										  0x27, 0x47, 0x4B, 0x1B, 0x07, 0xB2, 0x61, 0x91
									   },
			.value                   = (uint8_t [0xC0]) { "{\"thingName\":\"\"}" },
			.notify_write            = 1,
			.notify_read             = 0,
			.read_indication         = 0,
