...
static void cli_ble_hibernate_enable (void)
{
    ...
    if (dpm_ble_hibernate < 1)
        RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(),
                                    ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_DPM_BLE_HIBERNATE, MODE_ENABLE);
#endif
        return;
}

static void cli_ble_hibernate_disable (void)
{
#ifdef RM_MAP_PERSISTANT_W
    int dpm_ble_hibernate;
    ...
    if (dpm_ble_hibernate)
        RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(),
                                    ENV_GROUP_WIFIPROFILE,
                                    WIFI_PROFILE_DPM_BLE_HIBERNATE, MODE_DISABLE);
#endif
        return;
}
...
bool cmd_ble_hibernate_enable (int argc, char *argv[])
{
    FSP_PARAMETER_NOT_USED(argc);
    FSP_PARAMETER_NOT_USED(argv);

    cli_ble_hibernate_enable();

    return true;
}

bool cmd_ble_hibernate_disable (int argc, char *argv[])
{
    FSP_PARAMETER_NOT_USED(argc);
    FSP_PARAMETER_NOT_USED(argv);

    cli_ble_hibernate_disable();
