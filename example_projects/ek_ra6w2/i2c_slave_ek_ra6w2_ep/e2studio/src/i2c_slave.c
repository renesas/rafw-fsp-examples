 ...
 * private functions
 */
static fsp_err_t i2c_slave_write(void);
static fsp_err_t i2c_slave_read(void);
static void toggle_led(void);

/*******************************************************************************************************************//**
 * @brief     Initialize I2C master and slave driver.
 * @param[IN] None
 * @retval    FSP_SUCCESS       R_I2C_Master_W and Slave driver opened successfully.
...
fsp_err_t init_i2c_driver(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT ("R_I2C_MASTER_W_Open API failed \r\n");
        return err;
    }

    /* Open slave I2C channel */
    err = R_I2C_SLAVE_W_Open(&g_i2c_slave_ctrl, g_i2c_slave.p_cfg);
    if (FSP_SUCCESS != err)
    {
        /* Display failure message in RTT */
        APP_ERR_PRINT ("R_I2C_SLAVE_W_Open API failed \r\n");

        /* Slave Open unsuccessful closing master module */
    	if (FSP_SUCCESS != R_I2C_MASTER_W_Close(&g_i2c_master_ctrl))
        {
            /* Display failure message in RTT */
            APP_ERR_PRINT ("R_I2C_MASTER_W_Close API failed \r\n");
        }
    }
 ...
 *  @retval     FSP_SUCCESS    On successfully R_I2C_Mater_W and Slave Write/Read operation.
...
static fsp_err_t i2c_slave_write(void)
{
    ...
    write_err = R_I2C_MASTER_W_Read(&g_i2c_master_ctrl, g_master_buf, BUF_LEN, false);
    if (FSP_SUCCESS != write_err)
    {
        APP_ERR_PRINT("\r\n ** R_I2C_MASTER_W_Read API failed ** \r\n");
        return write_err;
    }
...
static fsp_err_t i2c_slave_read(void)
{
    ...
    read_err = R_I2C_MASTER_W_Write(&g_i2c_master_ctrl, g_master_buf, BUF_LEN, false);
    /* Handle error */
    if (FSP_SUCCESS != read_err)
    {
        APP_ERR_PRINT("\r\n ** R_I2C_MASTER_W_Write API failed ** \r\n");
        return read_err;
    }
    ...
    if (NULL != p_args)
    {
        switch(p_args->event)
        {
            case I2C_SLAVE_EVENT_RX_REQUEST:
            {
                ...
                err = R_I2C_SLAVE_W_Read(&g_i2c_slave_ctrl, g_slave_rx_buf, BUF_LEN);
                if(FSP_SUCCESS != err)
                {
            ...
            case I2C_SLAVE_EVENT_TX_REQUEST:
            {
                ...
                err = R_I2C_SLAVE_W_Write(&g_i2c_slave_ctrl, g_slave_tx_buf, BUF_LEN);
                if(FSP_SUCCESS != err)
                {
                	/* Update return error here */
                	g_slave_api_ret_err = err;
...
static void toggle_led(void)
{
    R_GPIO_W_PinWrite (&g_gpio_w_ctrl, (bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_10, LED_ON);
    R_BSP_SoftwareDelay(TOGGLE_DELAY, BSP_DELAY_UNITS_MILLISECONDS);
    R_GPIO_W_PinWrite (&g_gpio_w_ctrl, (bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_10, LED_OFF);
}
...
{
    R_GPIO_W_PinWrite (&g_gpio_w_ctrl, (bsp_io_port_pin_t) BSP_IO_PORT_00_PIN_10, led_state);
}

/*******************************************************************************************************************//**
 * This function is called to do closing of external i2c master and slave module using its HAL level API.
 * @brief     Close the I2C Master and Slave module. Handle the Error internally with Proper RTT Message.
...
void deinit_i2c_driver(void)
{
    ...
    err = R_I2C_MASTER_W_Close(&g_i2c_master_ctrl);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT(" ** R_I2C_MASTER_W_Close API failed ** \r\n");
    }

    /* close opened I2C slave module */
    err = R_I2C_SLAVE_W_Close(&g_i2c_slave_ctrl);
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT(" ** R_I2C_SLAVE_W_Close API failed ** \r\n");
    }
}

/*******************************************************************************************************************//**
 * @} (end addtogroup r_i2c_slave_w_ep)
 **********************************************************************************************************************/
