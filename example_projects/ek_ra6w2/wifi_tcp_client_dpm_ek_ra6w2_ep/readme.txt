...
The wifi_tcp_client_dpm_ek_ra6w2_ep application acts as a TCP client in DPM mode and 
establishes a connection with a TCP server running on the same Wi-Fi network.
It sends a packet every 10000 ms. An RTC timer is registered with DPM, 
which wakes up the RA6W2 every 10000 ms. The TCP client then sends a packet
and goes back to sleep.
It will also receive packets from the TCP server. If a packet is received while
in sleep mode, it will wake up the RA6W2, allowing the application to receive
