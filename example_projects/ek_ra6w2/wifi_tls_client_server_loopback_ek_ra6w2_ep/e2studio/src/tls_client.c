...
#define MESSAGE "Hello from TLS client1!"
#define MAX_COUNT 5
 ...
 */
static err_t client_recv(void *arg, struct altcp_pcb *pcb, struct pbuf *p, err_t err) //data send back from server to client
{
    (void)arg;
    if (!p)
    {
    ...
    APP_PRINT_INFO("TLS Client: Received echo message successfully %s \n\n",MESSAGE);
...
static err_t client_connected(void *arg, struct altcp_pcb *pcb, err_t err)
{
    ...
    APP_PRINT_INFO("Connected to server.\n");
