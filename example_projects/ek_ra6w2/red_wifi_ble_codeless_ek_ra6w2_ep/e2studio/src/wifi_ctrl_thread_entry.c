...
static void my_ap_sta_discon_handler(WIFIEvent_t * xEvent)
{
    ...
    if (disconnect_reason == WLAN_REASON_4WAY_HANDSHAKE_TIMEOUT)
    {
        ...
        {
            vee_data_uint = 1;
            RM_VEE_FLASH_W_RecordWrite(&g_vee0_ctrl, VEE_ID_WRONG_PASSWD_CNT, (uint8_t *) &vee_data_uint, 4);
            vTaskDelay(100);
            reset();
        }
    }
...
void wifi_ctrl_thread_entry(void * pvParameters)
{
    ...
    BaseType_t sem_err = xSemaphoreTake(g_atcmd_init_semaphore, pdMS_TO_TICKS(7500));
   #else
