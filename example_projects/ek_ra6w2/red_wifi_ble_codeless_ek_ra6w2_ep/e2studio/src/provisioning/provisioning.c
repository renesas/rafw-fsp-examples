...
static fsp_err_t provisioning_station_connect_concurrent(char *p_input_ssid, char *p_input_psk);
...
static int32_t provisioning_mutex_delete(provisioning_mutex_t *p_mutex);
 ...
 * Functions
 **********************************************************************************************************************/
uint8_t is_valid_passphrase(const char *pw, size_t len)
{
    if (pw == NULL)
    {
        return false;
    ...
    if (len < MIN_PASSKEY_LEN || len > MAX_PASSKEY_LEN)
            {
        PRINTF("Password: '%.*s' (len=%u)\n", (int) len, pw, (unsigned int) len);
        return false;
    }
    return true;
}

void ap_ssid_and_psk_set_and_reboot(void)

{
    ...
    memset(ap_info.password, 0, sizeof(ap_info.password));
    memset(tmp_psk, 0, 64 + 3);

    if (new_pw_ap == NULL || new_pw_ap_size == 0)
            {
        return;
    }
    else if (is_valid_passphrase(new_pw_ap, new_pw_ap_size))
    {
        memcpy(tmp_psk, new_pw_ap, new_pw_ap_size);
    }
    else
    {
        APRINTF_I("ERROR : Something wrong.... PSK is not valid.\n");
        return;  // Skip configuring AP SSID if PSK is invalid.
    }

    memset(tmp_ssid, 0, 64 + 3);

    if (new_SSID_AP != NULL && new_SSID_AP_size <= MAX_SSID_LEN)
            {
        memcpy(tmp_ssid, new_SSID_AP, new_SSID_AP_size);
    } 
    else 
    {
        APRINTF_I("ERROR : Something wrong.... SSID is not valid.\n");
        return;  // Skip reboot
    }

    memcpy(ap_info.ssid, tmp_ssid, strlen(tmp_ssid));
    memcpy(ap_info.password, tmp_psk, strlen(tmp_psk));

    if (R_RED_SecureAssetStore(SECURE_ASSET_APP_INFO, (uint8_t *) &ap_info, sizeof(ap_info_t)) != FSP_SUCCESS)

    {
        APRINTF_I("Set [AP SSID , PW] failed\n");
    }
...
fsp_err_t provisioning_Open (void)
{
                                       ...
                                       long const factory_reset_flag)
{
    ...
    int buffer_size = 0;
#if defined(RM_MAP_PERSISTANT_W) || defined(__SUPPORT_USR_NVRAM__)
    char *tmp_thing = NULL;
#endif
...
#ifdef RM_MAP_PERSISTANT_W
    RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE, WIFI_PROFILE_SYS_MODE, (int) DFLT_SYSMODE);
                                            ...
                                            int32_t factory_reset_flag)
{
    ...
    switch (p_param->auth_type)
    {
        ...
        case eAPSecurityWEP:           ///< WEP
        {
            ...
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_WEPKEY0_0, tmp_psk);
#endif
            break;
        }
        ...
        case eAPSecurityWPA:           ///< WPA-PSK
        {
            ...
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_PROTO_0, proto_WPA);
            RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_ENC_TYPE_0);
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_ENCKEY_0, tmp_psk);
#endif
            break;
        }
        ...
        {
#ifdef RM_MAP_PERSISTANT_W
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_AUTH_TYPE_0, "OWE");
#endif
#ifdef RM_MAP_PERSISTANT_W
...
#ifdef RM_MAP_PERSISTANT_W
                RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_identity", eap_id);
#endif
            }
...
#ifdef RM_MAP_PERSISTANT_W
                RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_password", eap_pw);
#endif
            }
        ...
        default:
        {
            ...
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(),
                                             ...
                                             NVR_KEY_AUTH_TYPE_0,
                                             MODE_AUTH_WPA_PSK_STR);
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_PROTO_0, "WPA RSN");
            RM_MAP_PERSISTANT_W_Write_STRING(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_ENCKEY_0, tmp_psk);
#endif
            break;
        }
...
#ifdef RM_MAP_PERSISTANT_W
    RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFIPROFILE, WIFI_PROFILE_SYS_MODE, WIFI_DEVICE_MODE_EXT_STATION);
    set_sys_mode((int)eWiFiModeStation);
    RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_PROFILE_0, MODE_ENABLE);
#endif
    ...
    if (PROV_DPM_ON == p_param->dpm_mode)
    {
        ...
        if (provisioning_dpm_status_get(P_TYPE_SLEEP_MODE) != 3)
        {
            if (p_param->dpm_ka > 0)
            {
#ifdef RM_MAP_PERSISTANT_W
        ...
        else
        {
            printf("[SleepMode 3] Set DPM , KA : %d , UW : %d , TW : %d \n",
                   provisioning_dpm_status_get(P_TYPE_DPM_KEEP_ALIVE),
                   provisioning_dpm_status_get(P_TYPE_USER_WAKE_UP),
                   provisioning_dpm_status_get(P_TYPE_TIM_WAKE_UP));
    ...
    {
        printf(">>> Save Sleep mode to %d \n", provisioning_dpm_status_get(P_TYPE_SLEEP_MODE));
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(),
                                      ENV_GROUP_APPCFG,
        ...
        printf(">>> Save Sleep mode2 RTC time %d \n", provisioning_dpm_status_get(P_TYPE_RTC_TIME));
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Write_INT(RM_MAP_PERSISTANT_W_get_ctrl(),
                                      ENV_GROUP_APPCFG,
...
fsp_err_t provisioning_scan (int32_t scan_mode, provisioning_type_t provisioning_mode)
{
    ...
    scan_res_text = (char *) pvPortMalloc(CLI_SCAN_RSP_BUF_SIZE);
    if (NULL == scan_res_text)
    {
        printf("[%s] malloc failed. \n", __func__);
        if (NULL != scan_res_array)
        {
    ...
    if ((0 > status) || (0 == (strcmp(scan_res_text, "FAIL"))))
    {
    ...
    {
        if (NULL == strtok(NULL, "\t")) /* BSSID => Skip! */
        {
            break;
        }

        p = strtok(NULL, "\t");                             /* frequency */
        scan_res_array[scan_res_num].frequency = atoi(p);
        ...
        if (strstr(p, "WPA") != 0)
        {
            if (strstr(p, "EAP") != 0)
            {
                ...
                if (strstr(p, "WPA2-EAP-SUITE-B-192-GCMP-256") != 0)                       // WPA3-Enterprise 192Bits Mode
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecurityWPA3_EAP_192B; /* WPA3-ENT */
                }
                else
                {
                    /* <WPA2/WPA3-ENT> PMF 1    : WPA2-EAP-CCMP, EAP-SHA256-CCMP */
                    if ((strstr(p, "WPA2-EAP-CCMP") != 0) &&
                        (strstr(p, "EAP-SHA256-CCMP") != 0))
                    {
                        scan_res_array[scan_res_num].security_mode = eAPSecurityWPA2_AUTO_EAP; /* WPA2 WPA3-ENT */
                    }

                    /* <WPA3-ENT> PMF 2 : EAP-SHA256-CCMP : WPA3-Enterprise 128bits Only */
                    else if ((strstr(p, "EAP-SHA256-CCMP") != 0) &&
                            (strstr(p, "EAP-CCMP") == 0))
                    {
                        scan_res_array[scan_res_num].security_mode = eAPSecurityWPA3_EAP; /* WPA3-ENT */
                    }

                    /* <WPA2-ENT> PMF 1 or 0: WPA2-EAP-CCMP */
                    else if ((strstr(p, "WPA2-EAP-CCMP") != 0) &&
                            (strstr(p, "EAP-SHA256") == 0))
                    {
                        scan_res_array[scan_res_num].security_mode = eAPSecurityWPA2_EAP; /* WPA2-ENT */
                    }

                    /* <WPA1/2-ENT> PMF 0   : WPA-EAP-, WPA2-EAP- */
                    else if ((strstr(p, "WPA2-EAP") != 0) &&
                            (strstr(p, "WPA-EAP") != 0))
                    {
                        scan_res_array[scan_res_num].security_mode = eAPSecurityWPA_AUTO_EAP; /* WPA1 WPA2-ENT */
                    }

                    /* <WPA1-ENT> PMF 0 : WPA-EAP- : WPA1 Only */
                    else if ((strstr(p, "WPA-EAP") != 0) &&
                            (strstr(p, "WPA2-EAP") == 0))
                    {
                        scan_res_array[scan_res_num].security_mode = eAPSecurityWPA_EAP; /* WPA-ENT */
                    }
                    else
                    {
                        /* Unknown */
            ...
            else if (strstr(p, "WPA2-") != 0)
            {
                /* Start :__SUPPORT_WPA3_SAE__ */
                if (strstr(p, "+SAE") != 0)
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecurityRSN_SAE; /* WPA2 / SAE */
                }
                else if (strstr(p, "-SAE") != 0)
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecuritySAE;     /* SAE */
                }
                else
                ...
                else if (strstr(p, "-OWE") != 0)
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecurityOWE;      /* OWE */
                }
                else if (strstr(p, "WPA-") != 0)
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecurityWPA_AUTO; /* WPA / WPA2 */
                }
                else
                {
                    scan_res_array[scan_res_num].security_mode = eAPSecurityWPA2;     /* WPA2 */
                }
            }
            else
            {
        ...
        /* Discard hidden SSID. */
        if (HIDDEN_SSID_DETECTION_CHAR == p[0])
        {
            continue;
        }

        memcpy(scan_res_array[scan_res_num].p_ssid, p, strlen(p));
        printf("(%d) %s / %d / %d / %d \n",
               scan_res_num,
...

{
    ...
    wifi_err = WIFI_Ping(ipaddr, count, interval);
...
fsp_err_t provisioning_notify_connection_on_concurrent (int concurrent_status,
                                                        char *p_buf)
{
    fsp_err_t err = FSP_SUCCESS;

    RA6W1_UNUSED_ARG(p_buf);

    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

    switch (concurrent_status)
    {
        case 0:
        {
            station_conn_status = 1; try_connect_on_sta = 0;
            break;
        }

        case 1:
        {
            station_conn_status = 0; try_connect_on_sta = 0;
            break;
        }
        ...
        default:
        {
            printf("NotifyConnectionOnConcurrent concurrent_status error [%d]", concurrent_status);
            err = FSP_ERR_INVALID_ARGUMENT;
            break;
        }
    }
                                                    ...
                                                    int size)
{
    ...
    snprintf(p_ssid,
             (size_t) ((size >
             (PROV_MAX_SSID_LEN + 2)) ? (quotation ? (PROV_MAX_SSID_LEN + 2) : PROV_MAX_SSID_LEN) : (size)),
             "%s%s%s",
             quotation ? "\"" : "",
             p_prefix,
             quotation ? "\"" : "");
...
static int provisioning_dpm_status_get (provisioning_dpm_value_type_t value_type)
{
    ...
    {
        if (P_TYPE_SLEEP_MODE == value_type)
        {
            return sleep_mode_value;
        }

        if (P_TYPE_USE_DPM == value_type)
        {
            return dpm_mode_value;
        }

        if (P_TYPE_DPM_KEEP_ALIVE == value_type)
        {
            return dpm_keep_alive_time_value;
        }

        if (P_TYPE_USER_WAKE_UP == value_type)
        {
            return user_wakeup_time_value;
        }

        if (P_TYPE_TIM_WAKE_UP == value_type)
        {
            return tim_wakeup_time_value;
        }

        if (P_TYPE_RTC_TIME == value_type)
        {
...
/*******************************************************************************************************************//**
 * Connects to the Station AP when device is concurrent mode
 *
 * @param[in]  p_input_ssid         SSID for the Station AP.
 * @param[in]  p_input_psk          WPA-PSK key for Station AP.
 *
 * @retval FSP_SUCCESS              Function completed successfully.
 * @retval FSP_ERR_INVALID_ARGUMENT p_input_ssid is NULL.
 * @retval FSP_ERR_OUT_OF_MEMORY    Heap is too small or NULL to create a string buffer.
 **********************************************************************************************************************/
static fsp_err_t provisioning_station_connect_concurrent (char *p_input_ssid, char *p_input_psk)
{
    fsp_err_t err = FSP_SUCCESS;
    ...
    ra6w1_cli_reply("remove_network 0", NULL, value_str);

    /* Start new STA profile */
    ra6w1_cli_reply("add_network 0", NULL, value_str);

    /* Need to make string with SSID */
    printf("Concurrent SSID : %s PW : %s ", p_input_ssid, p_input_psk);

    memset(cmdline, 0, 128);
    sprintf(cmdline, "set_network 0 ssid '%s'", p_input_ssid);
    ra6w1_cli_reply(cmdline, NULL, value_str); // set_network 0 ssid 'NEW_SSID'

    if ((eAPSecurityWPA_EAP <= security_type) && (eAPSecurityWPA3_EAP_192B > security_type))
    {
        /* EAP ID */
        memset(cmdline, 0, 128);
        sprintf(cmdline, "set_network 0 identity '%s'", eap_ID);
        ra6w1_cli_reply(cmdline, NULL, value_str);

        /* EAP PW */
        memset(cmdline, 0, 128);
        sprintf(cmdline, "set_network 0 password '%s'", eap_PW);
        ra6w1_cli_reply(cmdline, NULL, value_str);

        /* Pairwiae: TKIP, CCMP, AUTO */
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_ENC_TYPE_0);
#endif

        /* Auth mode: WPA_EAP, WPA2_EAP, WPA_AUTO_EAP */
        ra6w1_cli_reply("set_network 0 key_mgmt WPA-EAP", NULL, value_str);   // WPA_EAP
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, NVR_KEY_PROTO_0);
#endif

        /* Pairwiae: TKIP, CCMP, AUTO */
        ra6w1_cli_reply("set_network 0 pairwise TKIP CCMP", NULL, value_str); // AUTO

        /* Auth type: eap: PEAP TTLS FAST, PEAP, TTLS, FAST, TLS */
        if (TYPE_EAP_PEAP == eap_type)
        {
            ra6w1_cli_reply("set_network 0 eap PEAP", NULL, value_str);           // PEAP
        }
        else if (TYPE_EAP_TTLS == eap_type)
        {
            ra6w1_cli_reply("set_network 0 eap TTLS", NULL, value_str);           // TTLS
        }
        else if (TYPE_EAP_FAST == eap_type)
        {
            ra6w1_cli_reply("set_network 0 eap FAST", NULL, value_str);           // FAST
        }
        else if (TYPE_EAP_TLS == eap_type)
        {
            ra6w1_cli_reply("set_network 0 eap TLS", NULL, value_str);            // TLS: Not Support
        }
        else
        {
            ra6w1_cli_reply("set_network 0 eap PEAP TTLS FAST", NULL, value_str); // PEAP TTLS FAST,  TYPE_EAP_DEAULT
        }
        ...
        /* Auth protocol: phase2: MSCHAPV2 GTC,MSCHAPV2, GTC */
        if (PROTO_EAP_MSCHAPV2 == eap_protocol)
        {
            ra6w1_cli_reply("set_network 0 phase2 'auth=MSCHAPV2'", NULL, value_str);     // MSCHAPV2
        }
        else if (PROTO_EAP_GTC == eap_protocol)
        {
            ra6w1_cli_reply("set_network 0 phase2 'auth=GTC'", NULL, value_str);          // GTC
        }
        else
        {
            ra6w1_cli_reply("set_network 0 phase2 'auth=MSCHAPV2 GTC'", NULL, value_str); // MSCHAPV2 GTC
        }
    }

    if (eAPSecurityOWE == security_type)                                                   ///< WPA3 OWE
    {
        ra6w1_cli_reply("set_network 0 key_mgmt OWE", NULL, value_str);
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key0");
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key1");
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key2");
    ...
    else if ((eAPSecuritySAE == security_type) || (eAPSecurityRSN_SAE == security_type))
    {
        if (eAPSecuritySAE == security_type) ///< WPA3 SAE
        {
            ra6w1_cli_reply("set_network 0 key_mgmt SAE", NULL, value_str);
        }
        else                                 /// > WPA2 & WPA3 SAE //eAPSecurityRSN_SAE
        {
            ra6w1_cli_reply("set_network 0 key_mgmt WPA-PSK SAE", NULL, value_str);
        }

        ra6w1_cli_reply("set_network 0 proto RSN", NULL, value_str);
#ifdef RM_MAP_PERSISTANT_W
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key0");
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key1");
        RM_MAP_PERSISTANT_W_Erase(RM_MAP_PERSISTANT_W_get_ctrl(), ENV_GROUP_WIFICFG, "N0_wep_key2");
        ...
        memset(cmdline, 0, 128);
        sprintf(cmdline, "set_network 0 sae_password '%s'", p_input_psk);
        ra6w1_cli_reply(cmdline, NULL, value_str);
    }


    if ((NULL != p_input_psk) && (security_type <= eAPSecurityWPA_AUTO))
    {
        /* Need to make string with PSK */
        memset(cmdline, 0, 128);
        sprintf(cmdline, "set_network 0 psk '%s'", p_input_psk); // set_network 0 psk 'NEW_PSK'

        ra6w1_cli_reply(cmdline, NULL, value_str);

        /* Set for WPA/WPA2 PSK mode */
        ra6w1_cli_reply("set_network 0 key_mgmt WPA-PSK", NULL, value_str);

        /* Options : Don't need these fields mendatory... */
        /* Set Security protocol - Normally RSN for WPA2 */
        ra6w1_cli_reply("set_network 0 proto RSN", NULL, value_str); // WPA2
        /* Set Authenticate type */
        /* "TKIP" / "CCMP" / "TKIP CCMP" */
        ra6w1_cli_reply("set_network 0 pairwise CCMP", NULL, value_str);
    }

    /* Save profile in NVRAM */
    ra6w1_cli_reply("save_config 0", NULL, value_str);

    /* Connect STA with profile */
    ra6w1_cli_reply("select_network 0", NULL, value_str);
...
static provisioning_ap_cmd_type_t provisioning_parse_json_data (const char *p_received_data)
{
    ...
    ap_info_t ap_info_factory, ap_info_user;
    ...
    if (child_object != NULL)
    {
        ...
        if (SET_STA_SSID_PW == command)
        {
#if defined(CHECK_DEFAULT_SSID)
            if(R_RED_SecureAssetProdLoad(SECURE_ASSET_FACTORY_STORAGE_FLASH, SECURE_ASSET_APP_INFO, (uint8_t *) &ap_info_factory, sizeof(ap_info_t)) != FSP_SUCCESS)
            {
                APRINTF_I("No default ssid and pwd\n");
                return CMD_ERROR;
            }

            R_RED_SecureAssetLoad(SECURE_ASSET_APP_INFO, (uint8_t *) &ap_info_user, sizeof(ap_info_t));
            if ((0 == strcmp(ap_info_user.ssid, ap_info_factory.ssid)) && (0 == strcmp(ap_info_user.password, ap_info_factory.password)))
            {
                APRINTF_I("Same AP ssid or pwd\n");
                return CMD_ERROR;
            }
#endif                
#if defined(__PROVISION_ATCMD__)
        ...
        else if (child_object->valueint == SET_AP_SSID_PW)
        {
            APRINTF_I("json_recv_data = %s", p_received_data);
            cJSON *child_ssid = cJSON_GetObjectItem(json_recv_data, "ssid");
            cJSON *child_pw = cJSON_GetObjectItem(json_recv_data, "pw");

            memset(new_SSID_AP, 0, MAX_AP_SSID_SIZE);
            memset(new_pw_ap, 0, MAX_AP_PASSWORD_SIZE);
            new_SSID_AP_size = 0;
            new_pw_ap_size = 0;

            if (child_ssid != NULL)
            {
                new_SSID_AP_size = (int32_t) strlen(child_ssid->valuestring);
                strcpy(new_SSID_AP, child_ssid->valuestring);
            }

            if (child_pw != NULL)
            {
                new_pw_ap_size = (int32_t) strlen(child_pw->valuestring);
                strcpy(new_pw_ap, child_pw->valuestring);
            }
            APRINTF_I("[AP SET SSID %s, PW %s]\n",new_SSID_AP, new_pw_ap );
...
static void provisioning_client_thread(void *tls_info)
{
    ...
    while (1)
    {
        ...
        if (PROV_COMMAND_ERROR != status)
        {
            ...
            {
                /* Scan list update */
                err = provisioning_scan(1, prov_type);
            ...
            case SET_STA_SSID_PW:
            {
                ...
                if (0 != provisioning_dpm_status_get(P_TYPE_SLEEP_MODE))
                {
                    dpm_mode_on_reboot = (provisioning_dpm_mode_t) provisioning_dpm_status_get(P_TYPE_USE_DPM);
                    printf("Reset DPM mode to %d \n", dpm_mode_on_reboot);
                }
                if(WIFI_DEVICE_MODE_EXT_AP_STATION == get_run_mode())
                {
                    ...
                    if (0 != new_pw_size)
                    {
                    	if (0 != new_SSID_size)
                        {
                        err = provisioning_station_connect_concurrent(new_SSID,new_pw);
                        }
                        else
                        {
                ...
                else
                {
                    if (0 != new_SSID_size)
                    {
                        err = provisioning_station_connect_concurrent(new_SSID,NULL);
                    }
                    else
                    {
                        printf("ERROR : Something wrong.... SSID is null \n");
                ...
                else if(WIFI_DEVICE_MODE_EXT_AP == get_run_mode())
                {
                     printf("[Soft AP mode] \n");
                     if (0 != new_pw_size)
                     {
                ...
                case REQ_HOMEAP_RESULT:
                {
                    ...
                    {
                        printf("Waiting to complete AP connection ... \n");
            ...
            case REQ_REBOOT:
            {
                ...
                if (provisioning_dpm_status_get(P_TYPE_SLEEP_MODE) != 0)
            ...
            case REQ_SOCKET_TYPE:
            {
                ...
                {
                    if (PROV_TLS_SOCKET == provisioning_get_socket_type())    // tls
                    {
            ...
            case SET_AP_SSID_PW: 
            {
                ap_ssid_and_psk_set_and_reboot();
            }
                break;
...
static void provisioning_tcp_server_thread (void *tls_info)
{
    ...
    new_SSID_AP       = (char *) pvPortMalloc(MAX_AP_SSID_SIZE);
    new_pw             = (char *) pvPortMalloc(MAX_AP_PASSWORD_SIZE);
    new_pw_ap         = (char *) pvPortMalloc(MAX_AP_PASSWORD_SIZE);
