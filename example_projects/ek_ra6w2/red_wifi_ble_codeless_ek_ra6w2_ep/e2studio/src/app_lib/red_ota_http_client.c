...
* Description  : OTA http client functions
...
err_t red_ota_run_http_client(char *url, char *sdk_version, char *image_name,
                              char *mcu_fw_update, char *mcu_fw_image_name,
                              char *cert_key_update, char *cert_key_image_name);
                                         ...
                                         char *buffer, size_t bufferLength)
{
    ...
    {
        HTTPC_DEBUG_ERR("JSON_Search query %s error, result %d\n", query, result);
        return result;
    }
...
static fsp_err_t rm_ota_w_http_client_parse_request(const char *url, http_client_request_t *request)
{
    ...
    memcpy(request->path, url, strlen((const char *)url));
