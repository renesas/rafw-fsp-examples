...
void app_task_entry(void *pvParameters)
{
    ...
    fsp_err_t sensor_err = init_sensor();
    if (FSP_SUCCESS != sensor_err)
        APP_PRINT_ERR("[app_task] init_sensor failed: %d\n", sensor_err);
    else
        APP_PRINT_INFO("[app_task] Sensor initialized OK\n");

    vTaskDelay(pdMS_TO_TICKS(1000));

    if (FSP_SUCCESS == sensor_err)
    {
        http_update_sensor_data();
        APP_PRINT_INFO("[app_task] Initial sensor cache primed\n");
    }

    /* Start HTTPS server after sensor is ready */
    err = init_server();
    format_fs();
    ...
    while (1)
    {
        ...
        {
            sensor_err = init_sensor();
            if (FSP_SUCCESS == sensor_err)
            {
                APP_PRINT_INFO("[app_task] Sensor re-init OK\n");
