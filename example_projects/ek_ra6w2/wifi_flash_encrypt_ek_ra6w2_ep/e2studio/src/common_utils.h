...
#define DBG_PRINT(lvl, lvl_str, fmt, ...) \
    do { \
        if (lvl <= DBG_LVL) \
            rtt_printf(SEGGER_INDEX, "[" lvl_str "] " fmt, ##__VA_ARGS__); \
       } while(0)

#define DBG_PRINT_DIRECT(lvl, fmt, ...) \
    do { \
        if (lvl <= DBG_LVL) \
            rtt_printf(SEGGER_INDEX, fmt, ##__VA_ARGS__); \
       } while(0)
...
#define APP_ERR_PRINT(fn_, ...) if(DBG_LVL)\
                                   rtt_printf (SEGGER_INDEX, "[ERR] In Function: %s(), %s",__FUNCTION__,(fn_),##__VA_ARGS__);

#define APP_ERR_TRAP(err) if(err) {\
                                      rtt_printf(SEGGER_INDEX, "\r\nReturned Error Code: 0x%x  \r\n", err);\
                                      __asm("BKPT #0\n");} /* trap upon the error  */
