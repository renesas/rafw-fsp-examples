...
fsp_err_t set_timer_duty_cycle(uint8_t duty_cycle_percent)
{
    ...
    err = R_TIM_W_InfoGet(&g_timer_pwm_ctrl, &info);
    if (FSP_SUCCESS != err)
    {
        /* GPT Timer InfoGet Failure message */
        APP_ERR_PRINT("\r\n ** R_TIM_InfoGet API failed ** \r\n");
    }
    else
    {
        ...
        err = R_TIM_W_DutyCycleSet(&g_timer_pwm_ctrl, duty_cycle_counts, TIMER_PIN);
        if (FSP_SUCCESS != err)
        {
