 ...
 * Description  : NVRAM Encryption, CC312 + VEE Flash encryption.
 **********************************************************************************************************************/
/***********************************************************************************************************************
...
#define EP_APP_MODULE_NAME "rm_wifi"
#define EP_APP_DESCRIPTION \
    "This example demonstrates how to securely store and retrieve sensitive application data such as " \
    "Wi-Fi credentials and attestation keys using the CC312 Security Library. \r" \
    "It utilizes the VEE Flash interface with encryption to ensure data confidentiality and integrity."
...
void app_task_entry(void *pvParameters)
{
    ...
    */
    ...
    /* Freeze watchdog before long-running CC312 crypto + VEE flash operations
     * to prevent watchdog timeout during secure asset provisioning. */
    R_WDOG_W_Freeze(g_wifi_cfg.p_watchdog_service->p_cfg->p_wdt->p_ctrl, true);
    secure_asset_test_run();
