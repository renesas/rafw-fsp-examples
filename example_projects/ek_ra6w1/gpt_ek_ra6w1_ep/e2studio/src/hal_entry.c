            ...
            switch (read_data)
            {
                case PERIODIC_MODE_TIMER:
                {
                    ...
                    if (ONE_SHOT_MODE == g_timer_open_state)
                    {
                        ...
                        deinit_gpt_timer(&g_timer_oneshot_ctrl);
                    ...
                    if (gpt_desired_period_ms == 0)
                    {
                        /* Set it as default */
                        period_counts = TIMER_UNITS_DEFULT;
                    }
                    else
                    {
                        /* Convert period to PCLK counts so it can be set in hardware. */
                        period_counts = gpt_desired_period_ms * TIMER_UNITS_CONV_FACTOR;
                    }
                    ...
                    else
                    {
                        ...
                        err = R_TIM_W_PeriodSet(&g_timer_periodic_ctrl, (uint32_t) period_counts);
                        if (FSP_SUCCESS != err)
                        {
                            /* GPT Timer PeriodSet Failure message */
                            APP_ERR_PRINT("\r\n ** R_TIM_W_PeriodSet API failed ** \r\n");
                            /*Close Periodic Timer instance */
                            deinit_gpt_timer(&g_timer_periodic_ctrl);
                            APP_ERR_TRAP(err);
                        }
                ...
                case ONE_SHOT_MODE_TIMER:
                {
                    ...
                    err = init_gpt_timer(&g_timer_oneshot_ctrl, &g_timer_oneshot_cfg, ONE_SHOT_MODE_TIMER);
                    if (FSP_SUCCESS != err)
                    {
                        APP_ERR_PRINT("** GPT TIMER INIT FAILED ** \r\n");
                        APP_ERR_TRAP(err);
                    ...
                    err = start_gpt_timer(&g_timer_oneshot_ctrl);
                    if (FSP_SUCCESS != err)
                    {
                        APP_ERR_PRINT("** GPT TIMER START FAILED ** \r\n");
                        /*Close ONE-SHOT Timer instance */
