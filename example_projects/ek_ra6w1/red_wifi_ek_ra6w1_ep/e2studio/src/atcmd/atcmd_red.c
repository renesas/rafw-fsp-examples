...
* Description  : AT command for RED
...
#define RM_ATCMD_RED_CB(atcmd) \
    uint32_t RM_ATCMD_W_CORE_SECURE_STORAGE_ ## atcmd ## _cmd_cb(atcmd_w_ctrl_t * const p_at_ctrl, int argc, char *argv[])
...
RM_ATCMD_RED_CB(PRODASSETSTORE)
{
    ...
    else if (argc != 4)
    {
        ...
        printf("str leng: %u, %u, %u\n",
            (unsigned int) strlen(argv[1]),
            (unsigned int) strlen(argv[2]),
            (unsigned int) strlen(argv[3]));
...
RM_ATCMD_RED_CB(OTAMODE)
{
    ...
    if (argc == 1)
    {
        ...
        int result_len = sprintf(result_str, "%s:%u, %lu, %s\r\n",
