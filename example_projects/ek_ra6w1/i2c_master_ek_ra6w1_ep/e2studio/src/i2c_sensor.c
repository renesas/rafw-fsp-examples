...
/*******************************************************************************************************************//**
 *  @brief       initialize I2C master module and set up PMOD ACL sensor
 *  @param[IN]   None
...
fsp_err_t init_sensor(void)
{
    ...
    if ((FSP_SUCCESS == err) && (DEVICE_SIGNATURE == device_id))
    {
        ...
        err = R_I2C_MASTER_W_Write(&g_i2c_master_ctrl, measure_enable_payload, TWO_BYTE, false);
        if (FSP_SUCCESS == err)
        {
        ...
        else
        {
            ...
            APP_ERR_PRINT("** R_I2C_MASTER_W_Write API failed ** \r\n");
        }
    }
    /* handle error */
    else
...
/*******************************************************************************************************************//**
 *  @brief       DeInitialize I2C master module
 *  @param[IN]   None
 *  @retval      None
 **********************************************************************************************************************/
...
fsp_err_t read_sensor_data(uint8_t *xyz_data)
{
    ...
    err = R_I2C_MASTER_W_Write(&g_i2c_master_ctrl, &internal_reg, ONE_BYTE, true);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** R_I2C_MASTER_W_Write API failed ** \r\n");
    ...
    else
    {
        ...
        if (FSP_SUCCESS == err)
        {
            err = R_I2C_MASTER_W_Read(&g_i2c_master_ctrl, &adxl_data[0], DATA_REGISTERS, false);
            /* handle error */
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("** R_I2C_MASTER_W_Read API failed ** \r\n");
...
static fsp_err_t get_device_id(uint8_t *dev_id)
{
    ...
    err = R_I2C_MASTER_W_Write(&g_i2c_master_ctrl, &reg_address, ONE_BYTE, true);
    /* handle error */
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("** R_I2C_MASTER_W_Write API failed ** \r\n");
    ...
    else
    {
        ...
        else
        {
            ...
            err = R_I2C_MASTER_W_Read(&g_i2c_master_ctrl, dev_id, ONE_BYTE, false);
            /* handle error */
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("** R_I2C_MASTER_Read API failed ** \r\n");
...
/*******************************************************************************************************************//**
 * @} (end addtogroup r_i2c_master_w_ep)
 **********************************************************************************************************************/
