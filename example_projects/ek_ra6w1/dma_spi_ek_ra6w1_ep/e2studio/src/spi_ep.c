...
fsp_err_t spi_init(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        /* Close SPI slave */
        if ((FSP_SUCCESS != R_SPI_W_Close(&g_spi_slave_ctrl)))
        {
            /* SPI Slave Close Failure message */
            APP_ERR_PRINT("** R_SPI_W_Close API for SPI Slave failed ** \r\n");
        }
        /* Close SPI master */
        if ((FSP_SUCCESS != R_SPI_W_Close(&g_spi_master_ctrl)))
        {
...
void spi_clean_up(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        ...
        APP_ERR_PRINT("** R_SPI_W_Close API for master failed **  \r\n");
    }
    ...
    {
        /* SPI Close failure message */
        APP_ERR_PRINT("** R_SPI_W_Close API for slave failed **  \r\n");
    }
}
