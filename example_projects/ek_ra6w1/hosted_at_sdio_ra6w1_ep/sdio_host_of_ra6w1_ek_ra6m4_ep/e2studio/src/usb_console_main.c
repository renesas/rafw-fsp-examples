...
* Description  : USB console main entry function.
***********************************************************************************************************************/
/***********************************************************************************************************************
...
void usb_console_main(void)
{
    ...
    /* Initialize URART2, ICU, SPI modules */
...
uint8_t                 s_response[READ_BUF_SIZE]     = {0};
...
void usb_console_main(void)
{
    ...
    while (true) {
        EventBits_t uxBits;
        uxBits = xEventGroupWaitBits(g_update_console_event,  STATUS_USB_EVENT, pdTRUE, pdTRUE, portMAX_DELAY);
        if ((uxBits & (STATUS_USB_EVENT)) != (STATUS_USB_EVENT))
            continue;
        ...
        switch (g_basic0_ctrl.event) {
            ...
            case USB_STATUS_READ_COMPLETE:
            {
                ...
                if ((uxBits & (STATUS_CONSOLE_RX_EVENT)) != (STATUS_CONSOLE_RX_EVENT)) {
                    xResult = xEventGroupSetBitsFromISR(g_update_console_event, STATUS_CONSOLE_RX_EVENT,
                                                        &xHigherPriorityTaskWoken);
...
static fsp_err_t check_for_write_complete(void)
{
    xEventGroupWaitBits(g_update_console_event, STATUS_WRITE_COMPLETE, pdTRUE, pdTRUE, portMAX_DELAY);
    return FSP_SUCCESS;
}
/**********************************************************************************************************************
